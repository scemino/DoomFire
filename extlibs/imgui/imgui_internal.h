...
typedef int ImGuiColumnsFlags; // -> enum ImGuiColumnsFlags_       // Flags:
                               // BeginColumns()
...
IMGUI_API ImU32 ImHashStr(
    const char* data, size_t data_size = 0, ImU32 seed = 0);
...
IMGUI_API const char* ImStrchrRange(
    const char* str_begin, const char* str_end, char c);
IMGUI_API int ImStrlenW(const ImWchar* str);
IMGUI_API const char* ImStreolRange(
    const char* str, const char* str_end); // End end-of-line
...
IMGUI_API int ImFormatStringV(
    char* buf, size_t buf_size, const char* fmt, va_list args) IM_FMTLIST(3);
IMGUI_API const char* ImParseFormatFindStart(const char* format);
IMGUI_API const char* ImParseFormatFindEnd(const char* format);
IMGUI_API const char* ImParseFormatTrimDecorations(
    const char* format, char* buf, size_t buf_size);
...
IMGUI_API int ImTextStrFromUtf8(ImWchar* buf, int buf_size, const char* in_text,
...
IMGUI_API ImU64 ImFileRead(
    void* data, ImU64 size, ImU64 count, ImFileHandle file);
IMGUI_API ImU64 ImFileWrite(
...
IMGUI_API ImVec2 ImBezierCalc(const ImVec2& p1, const ImVec2& p2,
    const ImVec2& p3, const ImVec2& p4, float t); // Cubic Bezier
IMGUI_API ImVec2 ImBezierClosestPoint(const ImVec2& p1, const ImVec2& p2,
...
IMGUI_API ImVec2 ImBezierClosestPointCasteljau(const ImVec2& p1,
    const ImVec2& p2, const ImVec2& p3, const ImVec2& p4, const ImVec2& p,
    float tess_tol); // For auto-tessellated curves you can use tess_tol =
                     // style.CurveTessellationTol
IMGUI_API ImVec2 ImLineClosestPoint(
    const ImVec2& a, const ImVec2& b, const ImVec2& p);
IMGUI_API bool ImTriangleContainsPoint(
    const ImVec2& a, const ImVec2& b, const ImVec2& c, const ImVec2& p);
IMGUI_API ImVec2 ImTriangleClosestPoint(
    const ImVec2& a, const ImVec2& b, const ImVec2& c, const ImVec2& p);
...
#define IM_DRAWLIST_CIRCLE_AUTO_SEGMENT_CALC(_RAD, _MAXERROR)                \
    ImClamp((int)((IM_PI * 2.0f) / ImAcos(((_RAD) - (_MAXERROR)) / (_RAD))), \
...
struct IMGUI_API ImDrawListSharedData
{
    ...
    ImVec2 ArcFastVtx
        [12 * IM_DRAWLIST_ARCFAST_TESSELLATION_MULTIPLIER]; // FIXME: Bake
                                                            // rounded corners
                                                            // fill/borders in
                                                            // atlas
    ImU8 CircleSegmentCounts[64]; // Precomputed segment count for given radius
                                  // (array index + 1) before we calculate it
...
enum ImGuiItemStatusFlags_
{
    ...
    ImGuiItemStatusFlags_ToggledSelection =
        1 << 3, // Set when Selectable(), TreeNode() reports toggling a
                // selection. We can't report "Selected" because reporting the
                // change allows us to handle clipping with less issues.
...
enum ImGuiButtonFlags_
{
    ...
    ImGuiButtonFlags_PressedOnClick =
        1 << 1, // return true on click (mouse down event)
    ImGuiButtonFlags_PressedOnClickRelease =
        1 << 2, // [Default] return true on click + release on same item <--
                // this is what the majority of Button are using
    ImGuiButtonFlags_PressedOnClickReleaseAnywhere =
        1 << 3, // return true on click + release even if the release event is
                // not done while hovering the item
    ImGuiButtonFlags_PressedOnRelease =
        1 << 4, // return true on release (default requires click+release)
    ImGuiButtonFlags_PressedOnDoubleClick =
        1 << 5, // return true on double-click (default requires click+release)
    ...
    ImGuiButtonFlags_FlattenChildren =
        1 << 7, // allow interactions even if a child window is overlapping
    ImGuiButtonFlags_AllowItemOverlap =
        1
        << 8, // require previous frame HoveredId to either match id or be null
              // before being usable, use along with SetItemAllowOverlap()
    ImGuiButtonFlags_DontClosePopups =
        1
        << 9, // disable automatically closing parent popup on press // [UNUSED]
    ImGuiButtonFlags_Disabled = 1 << 10, // disable interactions
    ImGuiButtonFlags_AlignTextBaseLine =
        1
        << 11, // vertically align button to match text baseline - ButtonEx()
               // only // FIXME: Should be removed and handled by SmallButton(),
               // not possible currently because of DC.CursorPosPrevLine
    ImGuiButtonFlags_NoKeyModifiers =
        1 << 12, // disable mouse interaction if a key modifier is held
    ImGuiButtonFlags_NoHoldingActiveId =
        1 << 13, // don't set ActiveId while holding the mouse
                 // (ImGuiButtonFlags_PressedOnClick only)
    ImGuiButtonFlags_NoNavFocus =
        1 << 14, // don't override navigation focus when activated
    ImGuiButtonFlags_NoHoveredOnFocus =
        1 << 15, // don't report as hovered when nav focus is on this item
    ImGuiButtonFlags_MouseButtonLeft =
        1 << 16, // [Default] react on left mouse button
    ImGuiButtonFlags_MouseButtonRight = 1 << 17, // react on right mouse button
    ImGuiButtonFlags_MouseButtonMiddle = 1
                                         << 18, // react on center mouse button
...
struct IMGUI_API ImGuiInputTextState
{
    ...
    int CurLenW,
        CurLenA; // we need to maintain our buffer length in both UTF-8 and
                 // wchar format. UTF-8 length is valid even if TextA is not.
...
struct ImGuiNextWindowData
{
    ...
    ImVec2 MenuBarOffsetMinVal; // *Always on* This is not exposed publicly, so
                                // we don't clear it.
...
struct ImGuiSettingsHandler
{
    ...
    void (*ReadLineFn)(ImGuiContext* ctx, ImGuiSettingsHandler* handler,
        void* entry,
        const char*
            line); // Read: Called for every line of text within an ini entry
...
struct ImGuiContext
{
    ...
    ImGuiID TestEngineHookIdInfo;            // Will call test engine hooks:
    ...
    ImVector<ImGuiWindow*>
        WindowsFocusOrder; // Windows, sorted in focus order, back to front.
                           ...
                           // Need to sort out the Docking equivalent which is
                           // RootWindowDockStop and is unfortunately a little
                           // more dynamic)
    ImVector<ImGuiWindow*>
    ...
    ImU32 ActiveIdUsingNavInputMask; // Active widget will want to read those
                                     // nav inputs.
    ImU64
    ...
    ImRect NavScoringRect; // Rectangle used for scoring, in screen space. Based
                           // of window->DC.NavRefRectRel[], modified for
    ...
    ImGuiID
        NavInitResultId; // Init request result (first item of the window, or
    ...
    ImGuiWindow*
        NavWrapRequestWindow; // Window which requested trying nav wrap-around.
    ...
    int FocusRequestCurrCounterRegular;  // Any item being requested for focus,
                                         // stored as an index (we on layout to
                                         // be stable between the frame pressing
                                         // TAB and the next frame, semi-ouch)
    int FocusRequestCurrCounterTabStop;  // Tab item being requested for focus,
                                         // stored as an index
    ...
    ImDrawList ForegroundDrawList; // Last draw list to be rendered. This is
                                   // where we the render software mouse cursor
                                   // (if io.MouseDrawCursor is set) and most
                                   // debug overlays.
    ...
    float ColorEditLastHue; // Backup of last Hue associated to LastColor[3], so
          ...
          ForegroundDrawList(&DrawListSharedData)
    {
        ...
        NavJustTabbedId = NavJustMovedToId = NavJustMovedToFocusScopeId =
            NavNextActivateId = 0;
        ...
        FocusRequestCurrCounterRegular = FocusRequestCurrCounterTabStop =
            INT_MAX;
        FocusRequestNextCounterRegular = FocusRequestNextCounterTabStop =
            INT_MAX;
        ...
        ColorEditLastColor[0] = ColorEditLastColor[1] = ColorEditLastColor[2] =
            FLT_MAX;
...
struct IMGUI_API ImGuiWindowTempData
{
    ...
    ImVec2
        CursorMaxPos; // Used to implicitly calculate the size of our contents,
                      // always growing during the frame. Used to calculate
                      // window->ContentSize at the beginning of next frame
    ImVec2 CurrLineSize;
    ImVec2 PrevLineSize;
    ...
    ImGuiItemStatusFlags LastItemStatusFlags; // Status flags for last item (see
    ...
    int FocusCounterRegular; // (Legacy Focus/Tabbing system) Sequential
                             // counter, start at -1 and increase as assigned
                             // via FocusableItemRegister() (FIXME-NAV: Needs
                             // redesign)
...
struct IMGUI_API ImGuiWindow
{
    ...
    ImRect WorkRect;      // Cover the whole scrolling region, shrunk by
...
struct ImGuiTabItem
{
    ...
    float ContentWidth;    // Width of actual contents, stored during
                           // BeginTabItem() call
...
struct ImGuiTabBar
{
    ...
    ImRect BarRect;
    float LastTabContentHeight; // Record the height of contents submitted below
                                // the tab bar
    float OffsetMax; // Distance from BarRect.Min.x, locked during layout
    float
        OffsetMaxIdeal; // Ideal offset if all tabs were visible and not clipped
...
IMGUI_API void UpdateWindowParentAndRootLinks(
...
IMGUI_API void SetWindowPos(
    ImGuiWindow* window, const ImVec2& pos, ImGuiCond cond = 0);
IMGUI_API void SetWindowSize(
    ImGuiWindow* window, const ImVec2& size, ImGuiCond cond = 0);
IMGUI_API void SetWindowCollapsed(
    ImGuiWindow* window, bool collapsed, ImGuiCond cond = 0);
...
IMGUI_API ImVec2 ScrollToBringRectIntoView(
...
IMGUI_API bool ItemAdd(
    const ImRect& bb, ImGuiID id, const ImRect* nav_bb = NULL);
IMGUI_API bool ItemHoverable(const ImRect& bb, ImGuiID id);
IMGUI_API bool IsClippedEx(
    const ImRect& bb, ImGuiID id, bool clip_even_when_logged);
IMGUI_API bool FocusableItemRegister(
    ImGuiWindow* window, ImGuiID id); // Return true if focus is requested
...
IMGUI_API bool BeginChildEx(const char* name, ImGuiID id,
    const ImVec2& size_arg, bool border, ImGuiWindowFlags flags);
IMGUI_API void OpenPopupEx(
    ImGuiID id, ImGuiPopupFlags popup_flags = ImGuiPopupFlags_None);
IMGUI_API void ClosePopupToLevel(
...
IMGUI_API void NavMoveRequestTryWrapping(
...
IMGUI_API ImVec2 GetNavInputAmount2d(ImGuiNavDirSourceFlags dir_sources,
    ImGuiInputReadMode mode, float slow_factor = 0.0f,
    float fast_factor = 0.0f);
IMGUI_API int CalcTypematicRepeatAmount(
...
IMGUI_API void SetNavIDWithRectRel(
...
IMGUI_API bool IsMouseDragPastThreshold(
...
IMGUI_API void SetWindowClipRectBeforeSetChannel(
...
IMGUI_API float GetColumnOffsetFromNorm(
    const ImGuiColumns* columns, float offset_norm);
IMGUI_API float GetColumnNormFromOffset(
    const ImGuiColumns* columns, float offset);

// Tab Bars
IMGUI_API bool BeginTabBarEx(
    ImGuiTabBar* tab_bar, const ImRect& bb, ImGuiTabBarFlags flags);
IMGUI_API ImGuiTabItem* TabBarFindTabByID(ImGuiTabBar* tab_bar, ImGuiID tab_id);
IMGUI_API void TabBarRemoveTab(ImGuiTabBar* tab_bar, ImGuiID tab_id);
IMGUI_API void TabBarCloseTab(ImGuiTabBar* tab_bar, ImGuiTabItem* tab);
IMGUI_API void TabBarQueueChangeTabOrder(
...
IMGUI_API void RenderText(ImVec2 pos, const char* text,
    const char* text_end = NULL, bool hide_text_after_hash = true);
IMGUI_API void RenderTextWrapped(
...
IMGUI_API void RenderFrameBorder(
    ImVec2 p_min, ImVec2 p_max, float rounding = 0.0f);
...
IMGUI_API void LogRenderedText(
    const ImVec2* ref_pos, const char* text, const char* text_end = NULL);
...
IMGUI_API void RenderCheckMark(
...
// Widgets
IMGUI_API void TextEx(
    const char* text, const char* text_end = NULL, ImGuiTextFlags flags = 0);
...
IMGUI_API ImGuiID GetWindowResizeID(
    ImGuiWindow* window, int n); // 0..3: corners, 4..7: borders
...
IMGUI_API bool TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags,
    const char* label, const char* label_end = NULL);
...
IMGUI_API bool DragBehaviorT(ImGuiDataType data_type, T* v, float v_speed,
    T v_min, T v_max, const char* format, float power, ImGuiDragFlags flags);
template <typename T, typename SIGNED_T, typename FLOAT_T>
IMGUI_API bool SliderBehaviorT(const ImRect& bb, ImGuiID id,
    ImGuiDataType data_type, T* v, T v_min, T v_max, const char* format,
    float power, ImGuiSliderFlags flags, ImRect* out_grab_bb);
template <typename T, typename FLOAT_T>
IMGUI_API float SliderCalcRatioFromValueT(ImGuiDataType data_type, T v, T v_min,
    T v_max, float power, float linear_zero_pos);
...
// Color
IMGUI_API void ColorTooltip(
    const char* text, const float* col, ImGuiColorEditFlags flags);
IMGUI_API void ColorEditOptionsPopup(
    const float* col, ImGuiColorEditFlags flags);
IMGUI_API void ColorPickerOptionsPopup(
...
IMGUI_API void ImFontAtlasBuildPackCustomRects(
...
extern void ImGuiTestEngineHook_ItemAdd(
    ImGuiContext* ctx, const ImRect& bb, ImGuiID id);
...
extern void ImGuiTestEngineHook_IdInfo(ImGuiContext* ctx,
    ImGuiDataType data_type, ImGuiID id, const void* data_id);
extern void ImGuiTestEngineHook_IdInfo(ImGuiContext* ctx,
    ImGuiDataType data_type, ImGuiID id, const void* data_id,
...
#define IMGUI_TEST_ENGINE_LOG(_FMT, ...) \
    if(g.TestEngineHookItems)            \
    ImGuiTestEngineHook_Log(&g, _FMT,    \
...
#define IMGUI_TEST_ENGINE_ID_INFO2(_ID, _TYPE, _DATA, _DATA2) \
    if(g.TestEngineHookIdInfo == id)                          \
        ImGuiTestEngineHook_IdInfo(                           \
            &g, _TYPE, _ID, (const void*)(_DATA), (const void*)(_DATA2));
#else
#define IMGUI_TEST_ENGINE_ITEM_ADD(_BB, _ID) \
    do                                       \
    {                                        \
