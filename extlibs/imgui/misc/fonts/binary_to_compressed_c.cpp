...
int main(int argc, char** argv)
{
    ...
    bool ret = binary_to_compressed_c(
        argv[argn], argv[argn + 1], use_base85_encoding, use_compression);
    if(!ret)
...
static stb_uint stb_adler32(
    stb_uint adler32, stb_uchar* buffer, stb_uint buflen)
{
...
static int stb_compress_chunk(stb_uchar* history, stb_uchar* start,
    stb_uchar* end, int length, int* pending_literals, stb_uchar** chash,
    stb_uint mask)
{
    (void)history;
...
#define STB__TRY(t, p) /* avoid retrying a match we already tried */ \
    if(p ? dist != q - t : 1)                                        \
        if((m = stb_matchlen(t, q, match_max)) > best)               \
            if(stb__nc(m, q - (t))) best = m, dist = q - (t)
...
static int stb_compress_inner(stb_uchar* input, stb_uint length)
{
    ...
    for(i = 0; i < stb__hashsize; ++i) chash[i] = NULL;
