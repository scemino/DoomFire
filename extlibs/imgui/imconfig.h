...
// uses Dear ImGui. This is because some compile-time options have an affect on
// data structures. Defining those options in imconfig.h will ensure every
...
//IMGUI_DISABLE_METRICS_WINDOW                      // Disable debug/metrics
...
//ApplicationServices', this is why this is not the default). #define
//IMGUI_DISABLE_DEFAULT_FORMAT_FUNCTIONS            // Don't implement
//ImFormatString/ImFormatStringV so you can implement them yourself (e.g. if you
//don't want to link with vsnprintf) #define
...
//them yourself. #define IMGUI_DISABLE_DEFAULT_FILE_FUNCTIONS              //
//Don't implement ImFileOpen/ImFileClose/ImFileRead/ImFileWrite so you can
...
//---- Use 32-bit for ImWchar (default is 16-bit) to support full unicode code
//points. #define IMGUI_USE_WCHAR32
...
//---- Unless IMGUI_DISABLE_DEFAULT_FORMAT_FUNCTIONS is defined, use the much
//faster STB sprintf library implementation of vsnprintf instead of the one from
//the default C library.
// Note that stb_sprintf.h is meant to be provided by the user and available in
// the include path at compile time. Also, the compatibility checks of the
// arguments and formats done by clang and GCC will be disabled in order to
// support the extra formats provided by STB sprintf. #define
...
// back-ends support both 16/32-bit indices). Another way to allow large meshes
// while keeping 16-bit indices is to handle ImDrawCmd::VtxOffset in your
// renderer. Read about ImGuiBackendFlags_RendererHasVtxOffset for details.
