...
// for the default bindings files (imgui_impl_xxx.h) Using dear imgui via a
...
#define IM_FMTARGS(FMT)                \
    __attribute__((format(printf, FMT, \
...
#else
#define IM_OFFSETOF(_TYPE, _MEMBER) \
    ((size_t) & (((_TYPE*)0)->_MEMBER)) // Offset of _MEMBER within _TYPE. Old
                                        // style macro.
#endif
...
typedef void* ImTextureID; // User data for rendering back-end to identify a
...
IMGUI_API void
Render(); // ends the Dear ImGui frame, finalize the draw data. You can get call
          // GetDrawData() to obtain it and run your rendering function (up to
          // v1.60, this used to call io.RenderDrawListsFn(). Nowadays, we allow
          // and prefer calling your render function yourself.)
...
IMGUI_API void ShowMetricsWindow(
    bool* p_open =
        NULL); // create Debug/Metrics window. display Dear ImGui internals:
...
//   its return value [as with Begin: this is due to legacy reason and
//   inconsistent with most BeginXXX functions apart from the regular Begin()
...
IMGUI_API void SetNextWindowPos(const ImVec2& pos, ImGuiCond cond = 0,
    const ImVec2& pivot = ImVec2(
        0, 0)); // set next window position. call before Begin(). use
...
IMGUI_API void PushItemWidth(
    float item_width); // push width of items for common large "item+label"
                       ...
                       // to the right of window (so -1.0f always align width to
                       // the right side). 0.0f = default to ~2/3 of windows
                       // width,
...
GetCursorPosY(); //    other functions such as GetCursorScreenPos or everything
                 //    in ImDrawList::
IMGUI_API void SetCursorPos(const ImVec2&
...
IMGUI_API ImGuiID GetID(
    const char* str_id); // calculate unique ID (hash of whole ID stack + given
                         // parameter). e.g. if you want to query into
...
IMGUI_API bool Button(
    const char* label, const ImVec2& size = ImVec2(0, 0)); // button
IMGUI_API bool SmallButton(const char*
...
IMGUI_API bool ImageButton(ImTextureID user_texture_id, const ImVec2& size,
    ...
    const ImVec4& tint_col = ImVec4(
        1, 1, 1, 1)); // <0 frame_padding uses default frame padding settings. 0
                      // for no padding
IMGUI_API bool Checkbox(const char* label, bool* v);
IMGUI_API bool CheckboxFlags(
...
IMGUI_API void ProgressBar(float fraction,
    const ImVec2& size_arg = ImVec2(-1, 0), const char* overlay = NULL);
IMGUI_API void
...
IMGUI_API bool Combo(const char* label, int* current_item,
    ...
    int popup_max_height_in_items =
        -1); // Separate items with \0 within a string, end item-list with \0\0.
...
IMGUI_API bool DragScalar(const char* label, ImGuiDataType data_type,
    ...
    const void* p_max = NULL, const char* format = NULL, float power = 1.0f);
IMGUI_API bool DragScalarN(const char* label, ImGuiDataType data_type,
    void* p_data, int components, float v_speed, const void* p_min = NULL,
    const void* p_max = NULL, const char* format = NULL, float power = 1.0f);
...
IMGUI_API bool Selectable(const char* label, bool* p_selected,
    ImGuiSelectableFlags flags = 0,
    const ImVec2& size = ImVec2(
        0, 0)); // "bool* p_selected" point to the selection state (read-write),
...
IMGUI_API bool ListBoxHeader(
    const char* label, int items_count, int height_in_items = -1); // "
IMGUI_API void
...
IMGUI_API bool OpenPopupContextItem(const char* str_id = NULL,
    ImGuiPopupFlags popup_flags =
...
//  - We exceptionally default their flags to 1 (==
//  ImGuiPopupFlags_MouseButtonRight) for backward compatibility with older API
//  taking 'int mouse_button = 1' parameter. Passing a mouse button to
//  ImGuiPopupFlags is guaranteed to be legal.
...
IMGUI_API bool IsMouseReleased(ImGuiMouseButton
        button); // did mouse button released? (went from Down to !Down)
IMGUI_API bool IsMouseDoubleClicked(ImGuiMouseButton
        button); // did mouse button double-clicked? a double-click returns
...
enum ImGuiWindowFlags_
{
    ...
    // style.FrameBorderSize=1.0f or style.WindowBorderSize=1.0f to enable
    // borders around items or windows. ImGuiWindowFlags_ResizeFromAnySide    = 1
    // << 17,  // --> Set io.ConfigWindowsResizeFromEdges=true and make sure
    // mouse cursors are supported by back-end (io.BackendFlags &
...
enum ImGuiPopupFlags_
{
    ...
    ImGuiPopupFlags_MouseButtonLeft =
        0, // For BeginPopupContext*(): open on Left Mouse release. Guaranted to
           // always be == 0 (same as ImGuiMouseButton_Left)
    ImGuiPopupFlags_MouseButtonRight =
        1, // For BeginPopupContext*(): open on Right Mouse release. Guaranted
           // to always be == 1 (same as ImGuiMouseButton_Right)
    ImGuiPopupFlags_MouseButtonMiddle =
        2, // For BeginPopupContext*(): open on Middle Mouse release. Guaranted
    ...
    ImGuiPopupFlags_AnyPopupId = 1
                                 << 7, // For IsPopupOpen(): ignore the ImGuiID
...
enum ImGuiTabItemFlags_
{
    ...
    ImGuiTabItemFlags_NoCloseWithMiddleMouseButton =
        1 << 2, // Disable behavior of closing tabs (that are submitted with
                ...
                // this behavior on user's side with if (IsItemHovered() &&
                // IsMouseClicked(2)) *p_open = false.
    ImGuiTabItemFlags_NoPushId = 1 << 3, // Don't call PushID(tab->ID)/PopID()
...
enum ImGuiFocusedFlags_
{
    ...
    ImGuiFocusedFlags_AnyWindow =
        1 << 2, // IsWindowFocused(): Return true if any window is focused.
                ...
                // low-level inputs, do NOT use this. Use 'io.WantCaptureMouse'
...
enum ImGuiHoveredFlags_
{
    ...
    ImGuiHoveredFlags_AllowWhenOverlapped =
        1 << 6, // Return true even if the position is obstructed or overlapped
                // by another window
...
// io.KeyShift, io.KeyAlt set by user/back-end)
...
enum ImGuiBackendFlags_
{
    ...
    ImGuiBackendFlags_HasGamepad = 1 << 0, // Back-end Platform supports gamepad
                                           // and currently has one connected.
    ImGuiBackendFlags_HasMouseCursors =
        1 << 1, // Back-end Platform supports honoring GetMouseCursor() value to
                // change the OS cursor shape.
    ImGuiBackendFlags_HasSetMousePos =
        1 << 2, // Back-end Platform supports io.WantSetMousePos requests to
                // reposition the OS mouse position (only used if
                // ImGuiConfigFlags_NavEnableSetMousePos is set).
    ImGuiBackendFlags_RendererHasVtxOffset =
        1 << 3 // Back-end Renderer supports ImDrawCmd::VtxOffset. This enables
...
struct ImGuiStyle
{
    ...
    float TabMinWidthForUnselectedCloseButton; // Minimum width for close button
                                               // to appears on an unselected
                                               // tab when hovered. Set to 0.0f
                                               // to always show when hovering,
                                               // set to FLT_MAX to never show
    ...
    float CircleSegmentMaxError; // Maximum error (in pixels) allowed when using
                                 // AddCircle()/AddCircleFilled() or drawing
                                 // rounded corner rectangles with no explicit
                                 // segment count specified. Decrease for higher
...
struct ImGuiIO
{
    ...
    float ConfigWindowsMemoryCompactTimer; // = 60.0f          // [BETA] Compact
                                           // window memory usage when unused.
    ...
    IMGUI_API void AddInputCharacter(
        unsigned int c); // Queue new character input
    IMGUI_API void AddInputCharacterUTF16(
        ImWchar16 c); // Queue new character input from an UTF-16 character, it
                      // can be a surrogate
    ...
    bool
        MouseDownOwned[5]; // Track if button was clicked inside a dear imgui
                           // window. We don't request mouse capture from the
...
struct ImGuiInputTextCallbackData
{
    ...
    IMGUI_API void InsertChars(
        int pos, const char* text, const char* text_end = NULL);
...
static inline bool OpenPopupOnItemClick(
    const char* str_id = NULL, ImGuiMouseButton mb = 1)
{
    return OpenPopupContextItem(str_id, mb);
} // Passing a mouse button to ImGuiPopupFlags is legal
static inline bool BeginPopupContextWindow(
    const char* str_id, ImGuiMouseButton mb, bool over_items)
{
...
struct ImGuiTextFilter
{
    ...
    IMGUI_API bool PassFilter(
        const char* text, const char* text_end = NULL) const;
...
// rendering back-end accordingly.
#ifndef ImDrawCallback
typedef void (*ImDrawCallback)(
    const ImDrawList* parent_list, const ImDrawCmd* cmd);
#endif

// Special Draw callback value to request renderer back-end to reset the
// graphics/render state. The renderer back-end needs to handle this special
...
//   keeping 16-bit indices. Pre-1.71 back-ends will typically ignore the
...
// the renderer back-end (recommended). To use 32-bit indices: override with
...
// which can be drawn into out of order. This is used by the Columns api, so
// items of each column can be batched together in a same draw call.
...
struct ImDrawList
{
    ...
    ImDrawCmd _CmdHeader; // [Internal] Template of active commands. Fields
    ...
    //   higher-quality circles. Use AddNgon() and AddNgonFilled() functions if
    //   you need to guaranteed a specific number of sides.
    IMGUI_API void AddLine(
        const ImVec2& p1, const ImVec2& p2, ImU32 col, float thickness = 1.0f);
    IMGUI_API void AddRect(const ImVec2& p_min, const ImVec2& p_max, ImU32 col,
        float rounding = 0.0f,
    ...
    IMGUI_API void AddRectFilled(const ImVec2& p_min, const ImVec2& p_max,
        ImU32 col, float rounding = 0.0f,
    ...
    IMGUI_API void AddTriangle(const ImVec2& p1, const ImVec2& p2,
        const ImVec2& p3, ImU32 col, float thickness = 1.0f);
    IMGUI_API void AddTriangleFilled(
    ...
    IMGUI_API void AddCircleFilled(
        const ImVec2& center, float radius, ImU32 col, int num_segments = 12);
    IMGUI_API void AddNgon(const ImVec2& center, float radius, ImU32 col,
        int num_segments, float thickness = 1.0f);
    IMGUI_API void AddNgonFilled(
    ...
    IMGUI_API void AddImageQuad(ImTextureID user_texture_id, const ImVec2& p1,
...
struct ImFontAtlas
{
    ...
    IMGUI_API ImFont* AddFontFromMemoryTTF(void* font_data, int font_size,
        float size_pixels, const ImFontConfig* font_cfg = NULL,
    ...
    // e.g. custom colorful icons and use them as regular glyphs. Read
    // docs/FONTS.md for more details about using colorful icons. Note: this API
    // may be redesigned later in order to support multi-monitor varying DPI
    // settings.
    IMGUI_API int AddCustomRectRegular(int width, int height);
    IMGUI_API int AddCustomRectFontGlyph(ImFont* font, ImWchar id, int width,
        int height, float advance_x, const ImVec2& offset = ImVec2(0, 0));
...
struct ImFont
{
    ...
    ImWchar
        FallbackChar; // 2     // in  // = '?'      // Replacement character if
                      // a glyph isn't found. Only set via SetFallbackChar()
    ImWchar EllipsisChar;   // 2     // out // = -1       // Character used for
                            // ellipsis rendering.
    ...
    IMGUI_API ImVec2 CalcTextSizeA(float size, float max_width,
        float wrap_width, const char* text_begin, const char* text_end = NULL,
    ...
    IMGUI_API bool IsGlyphRangeUnused(
        unsigned int c_begin, unsigned int c_last);
};
