...
//  ImGuiBackendFlags_RendererHasVtxOffset flag. 2019-04-30: Metal: Added
//  support for special ImDrawCallback_ResetRenderState callback to reset render
//  state. 2019-02-11: Metal: Projecting clipping rectangles correctly using
...
//  in the About Window. 2018-07-05: Metal: Added new Metal backend
//  implementation.
...
// #import <QuartzCore/CAMetalLayer.h> // Not supported in XCode 9.2. Maybe a
// macro to detect the SDK version can be used (something like #if MACOS_SDK
// >= 10.13 ...)
#import <simd/simd.h>
...
void ImGui_ImplMetal_NewFrame(MTLRenderPassDescriptor* renderPassDescriptor)
{
    ...
    g_sharedMetalContext.framebufferDescriptor = [[FramebufferDescriptor alloc]
...
bool ImGui_ImplMetal_CreateFontsTexture(id<MTLDevice> device)
{
    ...
    io.Fonts->TexID =
        (__bridge void*)
            g_sharedMetalContext.fontTexture; // ImTextureID == void*
    ...
    (MTLRenderPassDescriptor*)renderPassDescriptor
{
    ...
    {
        _sampleCount =
            renderPassDescriptor.colorAttachments[0].texture.sampleCount;
        _colorPixelFormat =
...
// texture to save on memory access bandwidth. However, using a shader designed
// for 1-channel texture would make it less obvious to use the ImTextureID
// facility to render users own textures. You can make that change in your
// implementation.
- (void)makeFontTextureWithDevice:(id<MTLDevice>)device
{
    ImGuiIO& io = ImGui::GetIO();
...
- (_Nullable id<MTLRenderPipelineState>)renderPipelineStateForFrameAndDevice:
    (id<MTLDevice>)device
{
...
- (id<MTLRenderPipelineState>)
    _renderPipelineStateForFramebufferDescriptor:
                                          ...
                                          device:(id<MTLDevice>)device
{
    ...
    NSString* shaderSource =
        @""
         ...
         "[[buffer(1)]]) {\n"
         "    VertexOut out;\n"
         "    out.position = uniforms.projectionMatrix * float4(in.position, "
         "0, 1);\n"
      ...
      vertexBufferOffset:(size_t)vertexBufferOffset
{
    ...
    [commandEncoder
        setDepthStencilState:g_sharedMetalContext.depthStencilState];
    ...
    MTLViewport viewport = {.originX = 0.0,
        .originY = 0.0,
        .width =
            (double)(drawData->DisplaySize.x * drawData->FramebufferScale.x),
        .height =
        ...
        commandEncoder:(id<MTLRenderCommandEncoder>)commandEncoder
{
    ...
    MetalBuffer* vertexBuffer =
        [self dequeueReusableBufferOfLength:vertexBufferLength
                                     device:commandBuffer.device];
    MetalBuffer* indexBuffer =
        [self dequeueReusableBufferOfLength:indexBufferLength
                                     device:commandBuffer.device];
