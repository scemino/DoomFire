...
//  2019-04-03: Misc: Renamed imgui_impl_freeglut.cpp/.h to
...
void ImGui_ImplGLUT_KeyboardFunc(unsigned char c, int x, int y)
{
    ...
    // Store letters in KeysDown[] array as both uppercase and lowercase +
    // Handle GLUT translating CTRL+A..CTRL+Z as 1..26. This is a hacky mess but
    // GLUT is unable to distinguish e.g. a TAB key from CTRL+I so this is
    // probably the best we can do here.
    if(c >= 1 && c <= 26)
...
void ImGui_ImplGLUT_MouseFunc(int glut_button, int state, int x, int y)
{
    ...
    if(button != -1 && state == GLUT_DOWN) io.MouseDown[button] = true;
    if(button != -1 && state == GLUT_UP) io.MouseDown[button] = false;
}
