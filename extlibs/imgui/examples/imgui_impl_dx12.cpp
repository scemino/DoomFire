...
static void ImGui_ImplDX12_SetupRenderState(
    ImDrawData* draw_data, ID3D12GraphicsCommandList* ctx, FrameResources* fr)
{
...
void ImGui_ImplDX12_RenderDrawData(
    ImDrawData* draw_data, ID3D12GraphicsCommandList* ctx)
{
    ...
    for(int n = 0; n < draw_data->CmdListsCount; n++)
    {
        ...
        for(int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++)
        {
            ...
            else
            {
                ...
                const D3D12_RECT r = {(LONG)(pcmd->ClipRect.x - clip_off.x),
                ...
                ctx->SetGraphicsRootDescriptorTable(
                    1, *(D3D12_GPU_DESCRIPTOR_HANDLE*)&pcmd->TextureId);
...
static void ImGui_ImplDX12_CreateFontsTexture()
{
    ...
    // Upload texture to graphics system
    {
        ...
        g_pd3dDevice->CreateCommittedResource(&props, D3D12_HEAP_FLAG_NONE,
        ...
        HRESULT hr = g_pd3dDevice->CreateCommittedResource(&props,
        ...
        hr = g_pd3dDevice->CreateCommandQueue(
        ...
        g_pd3dDevice->CreateShaderResourceView(
            pTexture, &srvDesc, g_hFontSrvCpuDescHandle);
...
bool ImGui_ImplDX12_CreateDeviceObjects()
{
    ...
    // Create the root signature
    {
        ...
        if(D3D12SerializeRootSignature(
               &desc, D3D_ROOT_SIGNATURE_VERSION_1, &blob, NULL) != S_OK)
            return false;

        g_pd3dDevice->CreateRootSignature(0, blob->GetBufferPointer(),
            blob->GetBufferSize(), IID_PPV_ARGS(&g_pRootSignature));
        blob->Release();
    }
    ...
    // Create the vertex shader
    {
        ...
        if(FAILED(D3DCompile(vertexShader, strlen(vertexShader), NULL, NULL,
               NULL, "main", "vs_5_0", 0, 0, &vertexShaderBlob, NULL)))
            return false; // NB: Pass ID3D10Blob* pErrorBlob to D3DCompile() to
    ...
    // Create the pixel shader
    {
        ...
        if(FAILED(D3DCompile(pixelShader, strlen(pixelShader), NULL, NULL, NULL,
               "main", "ps_5_0", 0, 0, &pixelShaderBlob, NULL)))
        {
...
void ImGui_ImplDX12_InvalidateDeviceObjects()
{
    ...
    ImGuiIO& io = ImGui::GetIO();
    io.Fonts->TexID = NULL; // We copied g_pFontTextureView to io.Fonts->TexID
    ...
    D3D12_GPU_DESCRIPTOR_HANDLE font_srv_gpu_desc_handle)
{
    ...
    IM_UNUSED(cbv_srv_heap); // Unused in master branch (will be used by
                             // multi-viewports)
