...
//  2019-07-21: DirectX10: Backup, clear and restore Geometry Shader is any is
//  bound when calling ImGui_ImplDX10_RenderDrawData(). 2019-05-29: DirectX10:
//  Added support for large mesh (64K+ vertices), enable
//  ImGuiBackendFlags_RendererHasVtxOffset flag. 2019-04-30: DirectX10: Added
...
//  but removed in 1.47 (Nov 2015) on other back-ends. 2018-02-16: Misc:
...
static void ImGui_ImplDX10_SetupRenderState(
    ImDrawData* draw_data, ID3D10Device* ctx)
{
    // Setup viewport
    D3D10_VIEWPORT vp;
...
void ImGui_ImplDX10_RenderDrawData(ImDrawData* draw_data)
{
    ...
    if(!g_pVB || g_VertexBufferSize < draw_data->TotalVtxCount)
    {
        ...
        desc.MiscFlags = 0;
        if(ctx->CreateBuffer(&desc, NULL, &g_pVB) < 0) return;
    }
    ...
    if(!g_pIB || g_IndexBufferSize < draw_data->TotalIdxCount)
    {
        ...
        desc.CPUAccessFlags = D3D10_CPU_ACCESS_WRITE;
        if(ctx->CreateBuffer(&desc, NULL, &g_pIB) < 0) return;
    }
    ...
    // is (0,0) for single viewport apps.
    {
        ...
        if(g_pVertexConstantBuffer->Map(
               D3D10_MAP_WRITE_DISCARD, 0, &mapped_resource) != S_OK)
            return;
        VERTEX_CONSTANT_BUFFER* constant_buffer =
            (VERTEX_CONSTANT_BUFFER*)mapped_resource;
    ...
    ctx->IAGetIndexBuffer(
        &old.IndexBuffer, &old.IndexBufferFormat, &old.IndexBufferOffset);
    ...
    for(int n = 0; n < draw_data->CmdListsCount; n++)
    {
        ...
        for(int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++)
        {
            ...
            else
            {
                ...
                const D3D10_RECT r = {(LONG)(pcmd->ClipRect.x - clip_off.x),
                ...
                ID3D10ShaderResourceView* texture_srv =
                    (ID3D10ShaderResourceView*)pcmd->TextureId;
...
static void ImGui_ImplDX10_CreateFontsTexture()
{
    ...
    // Upload texture to graphics system
    {
        ...
        g_pd3dDevice->CreateShaderResourceView(
            pTexture, &srv_desc, &g_pFontTextureView);
        pTexture->Release();
    }
...
bool ImGui_ImplDX10_CreateDeviceObjects()
{
    ...
    if(g_pFontSampler) ImGui_ImplDX10_InvalidateDeviceObjects();
    ...
    // Create the vertex shader
    {
        ...
        if(FAILED(D3DCompile(vertexShader, strlen(vertexShader), NULL, NULL,
               NULL, "main", "vs_4_0", 0, 0, &vertexShaderBlob, NULL)))
        ...
        if(g_pd3dDevice->CreateVertexShader(
               ...
               vertexShaderBlob->GetBufferSize(), &g_pVertexShader) != S_OK)
        {
        ...
        D3D10_INPUT_ELEMENT_DESC local_layout[] = {
            {"POSITION", 0, DXGI_FORMAT_R32G32_FLOAT, 0,
                (UINT)IM_OFFSETOF(ImDrawVert, pos), D3D10_INPUT_PER_VERTEX_DATA,
                0},
            {"TEXCOORD", 0, DXGI_FORMAT_R32G32_FLOAT, 0,
                (UINT)IM_OFFSETOF(ImDrawVert, uv), D3D10_INPUT_PER_VERTEX_DATA,
                0},
            {"COLOR", 0, DXGI_FORMAT_R8G8B8A8_UNORM, 0,
                (UINT)IM_OFFSETOF(ImDrawVert, col), D3D10_INPUT_PER_VERTEX_DATA,
                0},
        };
        if(g_pd3dDevice->CreateInputLayout(local_layout, 3,
               vertexShaderBlob->GetBufferPointer(),
               vertexShaderBlob->GetBufferSize(), &g_pInputLayout) != S_OK)
        {
            vertexShaderBlob->Release();
            return false;
        }
    ...
    // Create the pixel shader
    {
        ...
        if(FAILED(D3DCompile(pixelShader, strlen(pixelShader), NULL, NULL, NULL,
               "main", "ps_4_0", 0, 0, &pixelShaderBlob, NULL)))
            return false; // NB: Pass ID3DBlob* pErrorBlob to D3DCompile() to
                          // get error showing in (const
                          // char*)pErrorBlob->GetBufferPointer(). Make sure to
                          // Release() the blob!
        if(g_pd3dDevice->CreatePixelShader(pixelShaderBlob->GetBufferPointer(),
               pixelShaderBlob->GetBufferSize(), &g_pPixelShader) != S_OK)
        {
...
{
    if(!g_pFontSampler) ImGui_ImplDX10_CreateDeviceObjects();
}
