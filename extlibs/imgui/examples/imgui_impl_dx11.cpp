...
static void ImGui_ImplDX11_SetupRenderState(
    ImDrawData* draw_data, ID3D11DeviceContext* ctx)
{
    // Setup viewport
    D3D11_VIEWPORT vp;
    ...
    ctx->HSSetShader(NULL, NULL, 0); // In theory we should backup and restore
                                     // this as well.. very infrequently used..
    ctx->DSSetShader(NULL, NULL, 0); // In theory we should backup and restore
                                     // this as well.. very infrequently used..
    ctx->CSSetShader(NULL, NULL, 0); // In theory we should backup and restore
                                     // this as well.. very infrequently used..
...
void ImGui_ImplDX11_RenderDrawData(ImDrawData* draw_data)
{
    ...
    if(!g_pVB || g_VertexBufferSize < draw_data->TotalVtxCount)
    {
        ...
        desc.MiscFlags = 0;
        if(g_pd3dDevice->CreateBuffer(&desc, NULL, &g_pVB) < 0) return;
    }
    if(!g_pIB || g_IndexBufferSize < draw_data->TotalIdxCount)
    {
        ...
        desc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
        if(g_pd3dDevice->CreateBuffer(&desc, NULL, &g_pIB) < 0) return;
    }
    ...
    // is (0,0) for single viewport apps.
    {
        ...
        if(ctx->Map(g_pVertexConstantBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0,
    ...
    for(int n = 0; n < draw_data->CmdListsCount; n++)
    {
        ...
        for(int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++)
        {
            ...
            else
            {
                ...
                const D3D11_RECT r = {(LONG)(pcmd->ClipRect.x - clip_off.x),
...
static void ImGui_ImplDX11_CreateFontsTexture()
{
    ...
    // Upload texture to graphics system
    {
        ...
        g_pd3dDevice->CreateShaderResourceView(
            pTexture, &srvDesc, &g_pFontTextureView);
        pTexture->Release();
    }
...
bool ImGui_ImplDX11_CreateDeviceObjects()
{
    ...
    if(g_pFontSampler) ImGui_ImplDX11_InvalidateDeviceObjects();
    ...
    // Create the vertex shader
    {
        ...
        D3D11_INPUT_ELEMENT_DESC local_layout[] = {
            {"POSITION", 0, DXGI_FORMAT_R32G32_FLOAT, 0,
                (UINT)IM_OFFSETOF(ImDrawVert, pos), D3D11_INPUT_PER_VERTEX_DATA,
                0},
            {"TEXCOORD", 0, DXGI_FORMAT_R32G32_FLOAT, 0,
                (UINT)IM_OFFSETOF(ImDrawVert, uv), D3D11_INPUT_PER_VERTEX_DATA,
                0},
            {"COLOR", 0, DXGI_FORMAT_R8G8B8A8_UNORM, 0,
                (UINT)IM_OFFSETOF(ImDrawVert, col), D3D11_INPUT_PER_VERTEX_DATA,
                0},
        };
    ...
    // Create the pixel shader
    {
        ...
        if(g_pd3dDevice->CreatePixelShader(pixelShaderBlob->GetBufferPointer(),
               pixelShaderBlob->GetBufferSize(), NULL, &g_pPixelShader) != S_OK)
        {
