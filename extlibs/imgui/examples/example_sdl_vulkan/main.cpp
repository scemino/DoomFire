...
static void SetupVulkan(const char** extensions, uint32_t extensions_count)
{
    ...
    // Create Vulkan Instance
    {
...
#ifdef IMGUI_VULKAN_DEBUG_REPORT
        // Enabling multiple validation layers grouped as LunarG standard
        // validation
        ...
        memcpy(
            extensions_ext, extensions, extensions_count * sizeof(const char*));
    ...
    // Select GPU
    {
        ...
        // for your purpose e.g. VK_PHYSICAL_DEVICE_TYPE_DISCRETE_GPU if
        // available, or with the greatest memory available, etc. for sake of
        // simplicity we'll just take the first one, assuming it has a graphics
        // queue family.
    ...
    ImGui_ImplVulkanH_Window* wd, VkSurfaceKHR surface, int width, int height)
{
    ...
    ImGui_ImplVulkanH_CreateOrResizeWindow(g_Instance, g_PhysicalDevice,
        g_Device, wd, g_QueueFamily, g_Allocator, width, height,
        g_MinImageCount);
}
...
int main(int, char**)
{
    ...
    while(!done)
    {
        ...
        while(SDL_PollEvent(&event))
        {
            ...
            {
                // Note: your own application may rely on
                // SDL_WINDOWEVENT_MINIMIZED/SDL_WINDOWEVENT_RESTORED to skip
                // updating all-together. Here ImGui_ImplSDL2_NewFrame() will
                // set io.DisplaySize to zero which will disable rendering but
                // let application run. Please note that you can't Present into
                // a minimized window.
        ...
        // Resize swap chain?
        if(g_SwapChainRebuild && g_SwapChainResizeWidth > 0 &&
            g_SwapChainResizeHeight > 0)
        {
            g_SwapChainRebuild = false;
            ImGui_ImplVulkan_SetMinImageCount(g_MinImageCount);
            ImGui_ImplVulkanH_CreateOrResizeWindow(g_Instance, g_PhysicalDevice,
                g_Device, &g_MainWindowData, g_QueueFamily, g_Allocator,
                g_SwapChainResizeWidth, g_SwapChainResizeHeight,
                g_MinImageCount);
