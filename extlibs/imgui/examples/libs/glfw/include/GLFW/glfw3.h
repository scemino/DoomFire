...
#define GLFW_DONT_CARE -1
     ...
     *  @param[in] mods Bit field describing which [modifier keys](@ref mods)
     * were held down.
     *
     ...
     *
     *  @param[in] window The window that received the event.
     *  @param[in] xpos The new cursor x-coordinate, relative to the left edge
     * of the client area.
     ...
     *
     *  This is the function signature for cursor enter/leave callback
     * functions.
     *
     *  @param[in] window The window that received the event.
     ...
     *
     *  @param[in] window The window that received the event.
     *  @param[in] key The [keyboard key](@ref keys) that was pressed or
     * released.
     ...
     *  callbacks.
     *
     *  This is the function signature for Unicode character with modifiers
     * callback functions.  It is called for each input character, regardless of
     * what modifier keys are held down.
     *
     *  @param[in] window The window that received the event.
     *  @param[in] codepoint The Unicode code point of the character.
     *  @param[in] mods Bit field describing which [modifier keys](@ref mods)
     ...
     *
     *  This is the function signature for monitor configuration callback
     * functions.
     *
     ...
     *
     *  If this function fails, it calls @ref glfwTerminate before returning. If
     * it succeeds, you should call @ref glfwTerminate before the application
     * exits.
     *
     ...
     * application's bundle, if present.  This can be disabled with a
     ...
     *  function is called, you must again call @ref glfwInit successfully
     ...
     * no need to call this function, as it is called by @ref glfwInit before it
     * returns failure.
     *
     *  @errors Possible errors include @ref GLFW_PLATFORM_ERROR.
     *
     ...
     * It describes the version, platform, compiler and any platform-specific
     *  compile-time options.  It should not be confused with the OpenGL or
     * OpenGL ES version string, queried with `glGetString`.
     *
     ...
     *
     *  The error callback is called on the thread where the error occurred.  If
     * you are using GLFW from multiple threads, your error callback needs to be
     *  written accordingly.
     *
     *  Because the description string may have been generated specifically for
     * that error, it is not guaranteed to be valid after the callback has
     * returned.  If you wish to use it after the callback returns, you need to
     * make a copy.
     *
     ...
     * no monitors were found, this function returns `NULL`.
     *
     *  @param[out] count Where to store the number of monitors in the returned
     *  array.  This is set to zero if an error occurred.
     *  @return An array of monitor handles, or `NULL` if no monitors were found
     * or if an [error](@ref error_handling) occurred.
     *
     *  @errors Possible errors include @ref GLFW_NOT_INITIALIZED.
     *
     *  @pointer_lifetime The returned array is allocated and freed by GLFW. You
     *  should not free it yourself.  It is guaranteed to be valid only until
     * the monitor configuration changes or the library is terminated.
     *
     *  @thread_safety This function must only be called from the main thread.
     *
     ...
     *
     *  Any or all of the position arguments may be `NULL`.  If an error occurs,
     * all non-`NULL` position arguments will be set to zero.
     *
     ...
     *  current resolution and system DPI instead of querying the monitor EDID
     * data.
     *
     *  @thread_safety This function must only be called from the main thread.
     *
    ...
    GLFWAPI void glfwGetMonitorPhysicalSize(
        GLFWmonitor* monitor, int* widthMM, int* heightMM);
     ...
     * You should not free it yourself.  It is valid until the specified monitor
     * is disconnected or the library is terminated.
     *
     *  @thread_safety This function must only be called from the main thread.
     *
     *  @sa @ref monitor_properties
     ...
     *  currently set callback.  This is called when a monitor is connected to
     ...
     *  disconnected, this function is called again for that monitor or the
     ...
     * this function is called and is restored by @ref glfwTerminate.
     *
     *  @param[in] monitor The monitor whose gamma ramp to set.
     ...
     *  GLFW_PLATFORM_ERROR.
     *
     *  @remark Gamma ramp sizes other than 256 are not supported by all
     * platforms or graphics hardware.
     *
    ...
    GLFWAPI void glfwSetGammaRamp(
        GLFWmonitor* monitor, const GLFWgammaramp* ramp);
     ...
     * by the next call to @ref glfwCreateWindow.
     *
     ...
     * you can use the newly created context, you need to [make it current](@ref
     * context_current).  For information about the `share` parameter, see @ref
     * context_sharing.
     *
     ...
     * information on how to query connected monitors, see @ref
     * monitor_monitors.
     *
     ...
     * not iconified, the supported video mode most closely matching the desired
     * video mode is set for the specified monitor.  For more information about
     * full screen windows, including the creation of so called _windowed full
     * screen_ or _borderless full screen_ windows, see @ref
     * window_windowed_full_screen.
     *
     *  By default, newly created windows use the placement recommended by the
     *  window system.  To create the window at a specific position, make it
     *  initially invisible using the [GLFW_VISIBLE](@ref window_hints_wnd)
     * window hint, set its [position](@ref window_pos) and then [show](@ref
     * window_hide) it.
     *
     ...
     * @ref GLFW_VERSION_UNAVAILABLE, @ref GLFW_FORMAT_UNAVAILABLE and @ref
     *  GLFW_PLATFORM_ERROR.
     *
     ...
     *  it will be set as the icon for the window.  If no such icon is present,
     * the `IDI_WINLOGO` icon will be used instead.
     *
     ...
     * icon. For more information on bundles, see the [Bundle Programming
     * Guide](https://developer.apple.com/library/mac/documentation/CoreFoundation/Conceptual/CFBundles/)
     *  in the Mac Developer Library.
     *
     *  @remark @osx The first time a window is created the menu bar is
     * populated with common commands like Hide, Quit and About.  The About
     * entry opens a minimal about dialog with information from the
     * application's bundle.  The menu bar can be disabled with a [compile-time
     * option](@ref compile_options_osx).
     *
     *  @remark @osx On OS X 10.10 and later the window frame will not be
     ...
     * `Info.plist`.  For more information, see [High Resolution Guidelines for
     * OS
     ...
     *
     *  @remark @x11 Due to the asynchronous nature of X11, it may take a moment
     * for a window to reach its requested state.  This means you may not be
     * able to query the final size, position or other attributes directly after
     * window creation.
     *
     *  @reentrancy This function must not be called from a callback.
     *
     ...
     *  GLFW_PLATFORM_ERROR.
     *
     *  @remark @osx The window title will not be updated until the next time
     * you process events.
     *
     *  @thread_safety This function must only be called from the main thread.
     *
    ...
    /*! @brief Retrieves the position of the client area of the specified
     * window.
     *
     ...
     * corner of the client area, or `NULL`.
     ...
     * upper-left corner of the client area of the specified windowed mode
     ...
     * you have very good reasons for doing so, as it will confuse and annoy the
     * user.
     *
     ...
     * if once it is made windowed.  If the window is not resizable, this
    ...
    GLFWAPI void glfwSetWindowAspectRatio(
        GLFWwindow* window, int numer, int denom);
    ...
    GLFWAPI void glfwGetWindowFrameSize(
        GLFWwindow* window, int* left, int* top, int* right, int* bottom);
     ...
     *  initially created.  Set the [GLFW_FOCUSED](@ref window_hints_wnd) to
     ...
     *  place the window client area.  The refresh rate is ignored when no
     ...
     *  client area.
     *  @param[in] width The desired with, in screen coordinates, of the client
     * area or video mode.
     *  @param[in] height The desired height, in screen coordinates, of the
     * client area or video mode.
     ...
     *
     *  This function sets the position callback of the specified window, which
     * is called when the window is moved.  The callback is provided with the
     * screen position of the upper-left corner of the client area of the
     * window.
     *
    ...
    GLFWAPI GLFWwindowposfun glfwSetWindowPosCallback(
        GLFWwindow* window, GLFWwindowposfun cbfun);
     ...
     *  @errors Possible errors include @ref GLFW_NOT_INITIALIZED.
     *
     *  @remark @osx Selecting Quit from the application menu will trigger the
    ...
    GLFWAPI GLFWwindowclosefun glfwSetWindowCloseCallback(
        GLFWwindow* window, GLFWwindowclosefun cbfun);
    ...
    GLFWAPI GLFWwindowrefreshfun glfwSetWindowRefreshCallback(
        GLFWwindow* window, GLFWwindowrefreshfun cbfun);
    ...
    GLFWAPI GLFWwindowfocusfun glfwSetWindowFocusCallback(
        GLFWwindow* window, GLFWwindowfocusfun cbfun);
    ...
    GLFWAPI GLFWwindowiconifyfun glfwSetWindowIconifyCallback(
        GLFWwindow* window, GLFWwindowiconifyfun cbfun);
     ...
     * designed on those platforms.  You can use the [window refresh
     * callback](@ref window_refresh) to redraw the contents of your window when
     ...
     *  it behaves exactly like @ref glfwPollEvents, i.e. the events in the
     * queue are processed and the function then returns immediately. Processing
     ...
     * necessary during such operations.
     *
     *  On some platforms, certain callbacks may be called outside of a call to
     * one of the event processing functions.
     *
     *  If no windows exist, this function returns immediately.  For
     * synchronization of threads in applications that do not create windows,
     ...
     * @ref glfwPollEvents, i.e. the events in the queue are processed and the
     ...
     * it. If sticky mouse buttons are enabled, a mouse button press will ensure
     * that
     *  @ref glfwGetMouseButton returns `GLFW_PRESS` the next time it is called
     ...
     *
     *  If the key is `GLFW_KEY_UNKNOWN`, the scancode is used instead,
     * otherwise the scancode is ignored.  If a non-printable key or (if the key
     * is `GLFW_KEY_UNKNOWN`) a scancode that maps to a non-printable key is
     ...
     *
     *  @pointer_lifetime The returned string is allocated and freed by GLFW.
     * You should not free it yourself.  It is valid until the next call to @ref
     ...
     *  `GLFW_RELEASE`.  The higher-level action `GLFW_REPEAT` is only reported
     * to the key callback.
     *
     *  If the `GLFW_STICKY_KEYS` input mode is enabled, this function returns
     *  `GLFW_PRESS` the first time you call it for a key that was pressed, even
     * if that key has already been released.
     *
     ...
     *  @param[in] key The desired [keyboard key](@ref keys). `GLFW_KEY_UNKNOWN`
     ...
     *  `GLFW_PRESS` the first time you call it for a mouse button that was
    ...
    /*! @brief Retrieves the position of the cursor relative to the client area
     * of the window.
     *
     ...
     *  @param[out] ypos Where to store the cursor y-coordinate, relative to the
     * to top edge of the client area, or `NULL`.
     *
     *  @errors Possible errors include @ref GLFW_NOT_INITIALIZED and @ref
     *  GLFW_PLATFORM_ERROR.
     *
    ...
    /*! @brief Sets the position of the cursor, relative to the client area of
     * the window.
     *
     ...
     * GLFW already provides the `GLFW_CURSOR_DISABLED` cursor mode that hides
     ...
     *
     *  @remark @x11 Due to the asynchronous nature of X11, it may take a moment
     * for the window focus event to arrive.  This means you may not be able to
     * set the cursor position directly after window creation.
     *
     *  @thread_safety This function must only be called from the main thread.
     *
     *  @sa @ref cursor_pos
     ...
     *  bits per channel.  They are arranged canonically as packed sequential
     ...
     * hotspot.
     *  @param[in] yhot The desired y-coordinate, in pixels, of the cursor
     * hotspot.
     ...
     *
     *  Returns a cursor with a [standard shape](@ref shapes), that can be set
     * for a window with @ref glfwSetCursor.
     *
     ...
     *
     *  This function sets the key callback of the specified window, which is
     ...
     * keyboard layout.  If you want to input text, use the [character
     * callback](@ref glfwSetCharCallback) instead.
     *
     *  When a window loses input focus, it will generate synthetic key release
     *  events for all pressed keys.  You can tell these events from
     * user-generated events by the fact that the synthetic ones are generated
     * after the focus loss event has been processed, i.e. after the [window
     * focus callback](@ref glfwSetWindowFocusCallback) has been called.
     *
     ...
     * `GLFW_KEY_UNKNOWN`, their state is not saved and so it cannot be queried
     * with @ref glfwGetKey.
     *
     ...
     * callback](@ref glfwSetKeyCallback) is not.  Characters do not map 1:1 to
     * physical keys, as a key may produce zero, one or more characters.  If you
     *  want to know whether a specific physical key was pressed or released,
     ...
     * OS X or Alt key on Windows.  There is a [character with modifiers
     * callback](@ref glfwSetCharModsCallback) that receives these events.
     *
     *  @param[in] window The window whose callback to set.
     *  @param[in] cbfun The new callback, or `NULL` to remove the currently set
     *  callback.
    ...
    GLFWAPI GLFWcharfun glfwSetCharCallback(
        GLFWwindow* window, GLFWcharfun cbfun);

    /*! @brief Sets the Unicode character with modifiers callback.
     *
     *  This function sets the character with modifiers callback of the
     * specified window, which is called when a Unicode character is input
     * regardless of what modifier keys are used.
     *
     *  The character with modifiers callback is intended for implementing
     * custom Unicode character input.  For regular Unicode text input, see the
     *  [character callback](@ref glfwSetCharCallback).  Like the character
     *  callback, the character with modifiers callback deals with characters
     * and is keyboard layout dependent.  Characters do not map 1:1 to physical
     * keys, as a key may produce zero, one or more characters.  If you want to
     * know whether a specific physical key was pressed or released, see the
    ...
    GLFWAPI GLFWcharmodsfun glfwSetCharModsCallback(
        GLFWwindow* window, GLFWcharmodsfun cbfun);
     ...
     *  [window focus callback](@ref glfwSetWindowFocusCallback) has been
     * called.
     *
     *  @param[in] window The window whose callback to set.
     *  @param[in] cbfun The new callback, or `NULL` to remove the currently set
    ...
    GLFWAPI GLFWmousebuttonfun glfwSetMouseButtonCallback(
        GLFWwindow* window, GLFWmousebuttonfun cbfun);
    ...
    GLFWAPI GLFWcursorposfun glfwSetCursorPosCallback(
        GLFWwindow* window, GLFWcursorposfun cbfun);
    ...
    GLFWAPI GLFWcursorenterfun glfwSetCursorEnterCallback(
        GLFWwindow* window, GLFWcursorenterfun cbfun);
    ...
    GLFWAPI GLFWscrollfun glfwSetScrollCallback(
        GLFWwindow* window, GLFWscrollfun cbfun);
     ...
     * the callback has returned.  If you wish to use them after the callback
    ...
    GLFWAPI GLFWdropfun glfwSetDropCallback(
        GLFWwindow* window, GLFWdropfun cbfun);
     ...
     *
     *  Querying a joystick slot with no device present is not an error, but
     * will cause this function to return `NULL`.  Call @ref glfwJoystickPresent
     * to check device presence.
     *
     *  @param[in] joy The [joystick](@ref joysticks) to query.
     ...
     *
     *  @pointer_lifetime The returned array is allocated and freed by GLFW. You
     *  should not free it yourself.  It is valid until the specified joystick
     * is disconnected, this function is called again for that joystick or the
     * library is terminated.
     *
     *  @thread_safety This function must only be called from the main thread.
     *
     ...
     * joystick. The returned string is allocated and freed by GLFW.  You should
     ...
     * joystick is disconnected, this function is called again for that joystick
     ...
     *  currently set callback.  This is called when a joystick is connected to
     * or disconnected from the system.
     *
     *  @param[in] cbfun The new callback, or `NULL` to remove the currently set
     *  callback.
     ...
     * contains or is convertible to a UTF-8 encoded string.  If the clipboard
     * is empty or if its contents cannot be converted, `NULL` is returned and a
     * @ref GLFW_FORMAT_UNAVAILABLE error is generated.
     *
     ...
     *  @pointer_lifetime The returned string is allocated and freed by GLFW.
     * You should not free it yourself.  It is valid until the next call to @ref
     *  glfwGetClipboardString or @ref glfwSetClipboardString, or until the
     * library is terminated.
     *
     *  @thread_safety This function must only be called from the main thread.
     *
     ...
     *  been set using @ref glfwSetTime, the timer measures time elapsed since
     ...
     *  @errors Possible errors include @ref GLFW_NOT_INITIALIZED.
     *
     *  @thread_safety This function may be called from any thread.  Reading of
     * the internal timer offset is not atomic.
     *
     *  @sa @ref time
     *
     *  @since Added in version 1.0.
     ...
     *
     *  @thread_safety This function may be called from any thread.  Writing of
     * the internal timer offset is not atomic.
     *
     *  @sa @ref time
     *
     ...
     * This is sometimes called _vertical synchronization_, _vertical retrace
     *  synchronization_ or just _vsync_.
     *
     *  Contexts that support either of the `WGL_EXT_swap_control_tear` and
     *  `GLX_EXT_swap_control_tear` extensions also accept negative swap
     * intervals, which allow the driver to swap even if a frame arrives a
     * little bit late. You can check for the presence of these extensions using
     * @ref glfwExtensionSupported.  For more information about swap tearing,
     ...
     *  swap interval set to whatever is the default on that platform.  This is
     * done because some swap interval extensions used by GLFW do not allow the
     * swap interval to be reset to zero once it has been set to a non-zero
     * value.
     *
     ...
     *
     *  This function does not apply to Vulkan.  If you are rendering with
     * Vulkan, see @ref glfwGetInstanceProcAddress, `vkGetInstanceProcAddr` and
     ...
     * check is performed by @ref glfwInit.
     *
     *  The availability of a Vulkan loader does not by itself guarantee that
     * window surface creation or even device creation is possible.  Call @ref
     *  glfwGetRequiredInstanceExtensions to check whether the extensions
     * necessary for Vulkan surface creation are available and @ref
     *  glfwGetPhysicalDevicePresentationSupport to check whether a queue family
     ...
     * successful, the list will always contains `VK_KHR_surface`, so if you
     * don't require any additional extensions you can pass this list directly
     ...
     * glfwVulkanSupported to check whether Vulkan is available.
     *
     ...
     *
     *  @remarks Additional extensions may be required by future versions of
     * GLFW. You should check if any extensions you wish to enable are already
     * in the returned array, as it is an error to specify an extension more
     * than once in the `VkInstanceCreateInfo` struct.
     *
     *  @pointer_lifetime The returned array is allocated and freed by GLFW. You
     *  should not free it yourself.  It is guaranteed to be valid only until
     ...
     * extension function for the specified instance.  If instance is set to
     * `NULL` it can return any function exported from the Vulkan loader,
     ...
     *  @ingroup vulkan
     */
    GLFWAPI GLFWvkproc glfwGetInstanceProcAddress(
        VkInstance instance, const char* procname);
     ...
     * built for.
     *
     *  If Vulkan or the required window surface creation instance extensions
     * are not available on the machine, or if the specified instance was not
     * created with the required extensions, this function returns `GLFW_FALSE`
     * and generates a @ref GLFW_API_UNAVAILABLE error.  Call @ref
     * glfwVulkanSupported to check whether Vulkan is available and @ref
     ...
     *  @ingroup vulkan
     */
    GLFWAPI int glfwGetPhysicalDevicePresentationSupport(
     ...
     *
     *  If the Vulkan loader was not found at initialization, this function
     * returns `VK_ERROR_INITIALIZATION_FAILED` and generates a @ref
     * GLFW_API_UNAVAILABLE error.  Call @ref glfwVulkanSupported to check
     * whether the Vulkan loader was found.
     *
     *  If the required window surface creation instance extensions are not
     *  available or if the specified instance was not created with these
     * extensions enabled, this function returns
     * `VK_ERROR_EXTENSION_NOT_PRESENT` and generates a @ref
     * GLFW_API_UNAVAILABLE error.  Call @ref glfwGetRequiredInstanceExtensions
     * to check what instance extensions are required.
     *
     *  The window surface must be destroyed before the specified Vulkan
     * instance. It is the responsibility of the caller to destroy the window
     * surface.  GLFW does not destroy it for you.  Call `vkDestroySurfaceKHR`
     ...
     *  allocator.
     *  @param[out] surface Where to store the handle of the surface.  This is
     * set to `VK_NULL_HANDLE` if an error occurred.
     ...
     *  GLFW_API_UNAVAILABLE and @ref GLFW_PLATFORM_ERROR.
     *
     *  @remarks If an error occurs before the creation call is made, GLFW
     * returns the Vulkan error code most appropriate for the error. Appropriate
     * use of
     *  @ref glfwVulkanSupported and @ref glfwGetRequiredInstanceExtensions
    ...
    GLFWAPI VkResult glfwCreateWindowSurface(VkInstance instance,
        GLFWwindow* window, const VkAllocationCallbacks* allocator,
