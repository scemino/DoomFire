...
static uSynergyBool sSendReply(uSynergyContext* context)
{
    ...
    // Send reply
    ret = context->m_sendFunc(
        context->m_cookie, context->m_replyBuffer, reply_len);
...
static void sSendMouseCallback(uSynergyContext* context)
{
    ...
    context->m_mouseCallback(context->m_cookie, context->m_mouseX,
        context->m_mouseY, context->m_mouseWheelX, context->m_mouseWheelY,
    ...
    uint16_t modifiers, uSynergyBool down, uSynergyBool repeat)
{
    ...
    // Send callback
    context->m_keyboardCallback(
        context->m_cookie, key, modifiers, down, repeat);
}
...
static void sSendJoystickCallback(uSynergyContext* context, uint8_t joyNum)
{
    ...
    context->m_joystickCallback(context->m_cookie, joyNum,
        context->m_joystickButtons[joyNum], sticks[0], sticks[1], sticks[2],
        sticks[3]);
}
...
static void sProcessMessage(uSynergyContext* context, const uint8_t* message)
{
    ...
    if(memcmp(message + 4, "Synergy", 7) == 0)
    {
        ...
        if(!sSendReply(context))
        {
            ...
            sTrace(
                context, "SendReply failed, trying to reconnect in a second");
        ...
        else
        {
            ...
            char buffer[256 + 1];
            sprintf(
                buffer, "Connected as client \"%s\"", context->m_clientName);
    ...
    else if(USYNERGY_IS_PACKET("DKUP"))
    {
        ...
        sSendKeyboardCallback(
            context, key, mod, USYNERGY_FALSE, USYNERGY_FALSE);
    }
    else if(USYNERGY_IS_PACKET("DGBT"))
    {
        ...
        if(joy_num < USYNERGY_NUM_JOYSTICKS)
        {
            ...
            context->m_joystickButtons[joy_num] =
                (message[9] << 8) | message[10];
    ...
    else if(USYNERGY_IS_PACKET("DCLP"))
    {
        ...
        //always coming from the server? 		1 uint32:	The total size of the
        //remaining 'string' (as per the Synergy %s string format (which is 1
        ...
        for(; num_formats; num_formats--)
        {
            ...
            if(context->m_clipboardCallback)
                context->m_clipboardCallback(
                    context->m_cookie, format, parse_msg, size);
...
static void sUpdateContext(uSynergyContext* context)
{
    ...
    if(context->m_receiveFunc(context->m_cookie,
           context->m_receiveBuffer + context->m_receiveOfs, receive_size,
           &num_received) == USYNERGY_FALSE)
    {
        /* Receive failed, let's try to reconnect */
        char buffer[128];
        sprintf(buffer,
            "Receive failed (%d bytes asked, %d bytes received), trying to "
    ...
    /*	If we didn't receive any data then we're probably still polling to get
       connected and therefore not getting any data back. To avoid overloading
       the system with a Synergy
        thread that would hammer on polling, we let it rest for a bit if there's
       no data. */
    ...
    for(;;)
    {
        /* Grab packet length and bail out if the packet goes beyond the end of
         * the buffer */
        ...
        memmove(context->m_receiveBuffer,
            context->m_receiveBuffer + packlen + 4,
    ...
    if(packlen > USYNERGY_RECEIVE_BUFFER_SIZE)
    {
        ...
        while(num_received != packlen)
        {
            ...
            int to_receive = buffer_left < USYNERGY_RECEIVE_BUFFER_SIZE
                                 ? buffer_left
                                 : USYNERGY_RECEIVE_BUFFER_SIZE;
            int ditch_received = 0;
            if(context->m_receiveFunc(context->m_cookie,
                   context->m_receiveBuffer, to_receive,
                   &ditch_received) == USYNERGY_FALSE)
            {
                /* Receive failed, let's try to reconnect */
                sTrace(
                    context, "Receive failed, trying to reconnect in a second");
                sSetDisconnected(context);
                context->m_sleepFunc(context->m_cookie, 1000);
                break;
            }
...
void uSynergySendClipboard(uSynergyContext* context, const char* text)
{
    ...
    if(text_length > max_length)
    {
        char buffer[128];
        sprintf(buffer,
            "Clipboard buffer too small, clipboard truncated at %d characters",
            max_length);
        sTrace(context, buffer);
    ...
    sAddUInt32(
        context, 4 + 4 + 4 + text_length); /* Rest of message size: numFormats,
                                              format, length, data */
