...
// Metal..) [ALPHA] Early bindings, not well tested. If you want a portable
// application, prefer using the GLFW or SDL platform bindings on Mac.

// Implemented features:
//  [X] Platform: Mouse cursor shape and visibility. Disable with
//  'io.ConfigFlags |= ImGuiConfigFlags_NoMouseCursorChange'. [X] Platform: OSX
//  clipboard is supported within core Dear ImGui (no specific code in this
//  back-end).
// Issues:
//  [ ] Platform: Keys are all generally very broken. Best using [event keycode]
//  and not [event characters]..
...
//  2020-05-25: Inputs: Added a fix for missing trackpad clicks when done with
//  "soft tap". 2019-12-05: Inputs: Added support for
//  ImGuiMouseCursor_NotAllowed mouse cursor. 2019-10-11: Inputs:  Fix using
//  Backspace key. 2019-07-21: Re-added clipboard handlers as they are not
//  enabled by default in core imgui.cpp (reverted 2019-05-18 change).
...
//  core imgui.cpp. 2019-05-11: Inputs: Don't filter character values before
...
bool ImGui_ImplOSX_Init()
{
    ...
    io.BackendFlags |=
        ImGuiBackendFlags_HasMouseCursors; // We can honor GetMouseCursor()
                                           // values (optional)
    // io.BackendFlags |= ImGuiBackendFlags_HasSetMousePos;          // We can
    // honor io.WantSetMousePos requests (optional, rarely used) io.BackendFlags
    // |= ImGuiBackendFlags_PlatformHasViewports;    // We can create
    // multi-viewports on the Platform side (optional) io.BackendFlags |=
    // ImGuiBackendFlags_HasMouseHoveredViewport; // We can set
    ...
    io.KeyMap[ImGuiKey_LeftArrow] =
        NSLeftArrowFunctionKey + offset_for_function_keys;
    io.KeyMap[ImGuiKey_RightArrow] =
        NSRightArrowFunctionKey + offset_for_function_keys;
    io.KeyMap[ImGuiKey_UpArrow] =
        NSUpArrowFunctionKey + offset_for_function_keys;
    io.KeyMap[ImGuiKey_DownArrow] =
        NSDownArrowFunctionKey + offset_for_function_keys;
    io.KeyMap[ImGuiKey_PageUp] = NSPageUpFunctionKey + offset_for_function_keys;
    io.KeyMap[ImGuiKey_PageDown] =
        NSPageDownFunctionKey + offset_for_function_keys;
    ...
    // Note that imgui.cpp also include default OSX clipboard handlers which can
    // be enabled by adding '#define
    // IMGUI_ENABLE_OSX_DEFAULT_CLIPBOARD_FUNCTIONS' in imconfig.h and adding
    // '-framework ApplicationServices' to your linker command-line. Since we
    // are already in ObjC land here, it is easy for us to add a clipboard
    // handler using the NSPasteboard api.
    ...
    io.GetClipboardTextFn = [](void*) -> const char* {
        ...
        if(![available isEqualToString:NSPasteboardTypeString]) return NULL;
...
static void ImGui_ImplOSX_UpdateMouseCursorAndButtons()
{
    ...
    else
    {
        ...
        [g_MouseCursors[g_MouseCursors[imgui_cursor] ? imgui_cursor
...
void ImGui_ImplOSX_NewFrame(NSView* view)
{
    ...
    // Setup time step
    if(g_Time == 0.0) g_Time = CFAbsoluteTimeGetCurrent();
...
{
    ImGuiIO& io = ImGui::GetIO();
    for(int n = 0; n < IM_ARRAYSIZE(io.KeysDown); n++) io.KeysDown[n] = false;
}
...
bool ImGui_ImplOSX_HandleEvent(NSEvent* event, NSView* view)
{
    ...
    if(event.type == NSEventTypeScrollWheel)
    {
        ...
        if(fabs(wheel_dx) > 0.0) io.MouseWheelH += wheel_dx * 0.1f;
        if(fabs(wheel_dy) > 0.0) io.MouseWheel += wheel_dy * 0.1f;
        return io.WantCaptureMouse;
    }

    // FIXME: All the key handling is wrong and broken. Refer to GLFW's
    // cocoa_init.mm and cocoa_window.mm.
    if(event.type == NSEventTypeKeyDown)
    {
        ...
        for(int i = 0; i < len; i++)
        {
            ...
            // We must reset in case we're pressing a sequence of special keys
            // while keeping the command pressed
            int key = mapCharacterToKey(c);
            if(key != -1 && key < 256 && !io.KeyCtrl) resetKeys();
            if(key != -1) io.KeysDown[key] = true;
        }
    ...
    if(event.type == NSEventTypeKeyUp)
    {
        ...
        for(int i = 0; i < len; i++)
        {
            int c = [str characterAtIndex:i];
            int key = mapCharacterToKey(c);
            if(key != -1) io.KeysDown[key] = false;
        }
        return io.WantCaptureKeyboard;
    }
    ...
    if(event.type == NSEventTypeFlagsChanged)
    {
        ...
        // We must reset them as we will not receive any keyUp event if they
        // where pressed with a modifier
        if((oldKeyShift && !io.KeyShift) || (oldKeyCtrl && !io.KeyCtrl) ||
            (oldKeyAlt && !io.KeyAlt) || (oldKeySuper && !io.KeySuper))
            resetKeys();
        return io.WantCaptureKeyboard;
    }
