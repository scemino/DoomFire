...
//  Read the FAQ about ImTextureID! [X] Platform: Clipboard support (from
//  Allegro 5.1.12) [X] Platform: Mouse cursor shape and visibility. Disable
...
//  and clipping rectangle. 2018-06-13: Renderer: Backup/restore transform and
//  clipping rectangle. 2018-06-11: Misc: Setup io.BackendFlags
...
void ImGui_ImplAllegro5_RenderDrawData(ImDrawData* draw_data)
{
    ...
    al_get_clipping_rectangle(
        &last_clip_x, &last_clip_y, &last_clip_w, &last_clip_h);
    ...
    for(int n = 0; n < draw_data->CmdListsCount; n++)
    {
        ...
        // Allegro's implementation of al_draw_indexed_prim() for DX9 is
        // completely broken. Unindex our buffers ourselves.
        // FIXME-OPT: Unfortunately Allegro doesn't support 32-bit packed colors
        // so we have to convert them to 4 float as well..
        static ImVector<ImDrawVertAllegro> vertices;
        vertices.resize(cmd_list->IdxBuffer.Size);
        for(int i = 0; i < cmd_list->IdxBuffer.Size; i++)
        {
            const ImDrawVert* src_v =
                &cmd_list->VtxBuffer[cmd_list->IdxBuffer[i]];
        ...
        for(int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++)
        {
            ...
            else
            {
                ...
                al_set_clipping_rectangle(pcmd->ClipRect.x - clip_off.x,
                    pcmd->ClipRect.y - clip_off.y,
                    pcmd->ClipRect.z - pcmd->ClipRect.x,
                    pcmd->ClipRect.w - pcmd->ClipRect.y);
                al_draw_prim(&vertices[0], g_VertexDecl, texture, idx_offset,
                    idx_offset + pcmd->ElemCount, ALLEGRO_PRIM_TRIANGLE_LIST);
            }
...
bool ImGui_ImplAllegro5_CreateDeviceObjects()
{
    ...
    al_set_new_bitmap_flags(
        ALLEGRO_MEMORY_BITMAP | ALLEGRO_MIN_LINEAR | ALLEGRO_MAG_LINEAR);
...
{
    if(g_ClipboardTextData) al_free(g_ClipboardTextData);
...
void ImGui_ImplAllegro5_Shutdown()
{
    ...
    if(g_VertexDecl) al_destroy_vertex_decl(g_VertexDecl);
    g_VertexDecl = NULL;
    ...
    switch(ev->type)
    {
        ...
        case ALLEGRO_EVENT_MOUSE_BUTTON_UP:
            if(ev->mouse.display == g_Display && ev->mouse.button <= 5)
                io.MouseDown[ev->mouse.button - 1] =
        ...
        case ALLEGRO_EVENT_KEY_UP:
            if(ev->keyboard.display == g_Display)
                io.KeysDown[ev->keyboard.keycode] =
...
{
    if(!g_Texture) ImGui_ImplAllegro5_CreateDeviceObjects();
    ...
    io.KeyCtrl = al_key_down(&keys, ALLEGRO_KEY_LCTRL) ||
                 al_key_down(&keys, ALLEGRO_KEY_RCTRL);
    io.KeyShift = al_key_down(&keys, ALLEGRO_KEY_LSHIFT) ||
                  al_key_down(&keys, ALLEGRO_KEY_RSHIFT);
    io.KeyAlt = al_key_down(&keys, ALLEGRO_KEY_ALT) ||
                al_key_down(&keys, ALLEGRO_KEY_ALTGR);
    io.KeySuper = al_key_down(&keys, ALLEGRO_KEY_LWIN) ||
