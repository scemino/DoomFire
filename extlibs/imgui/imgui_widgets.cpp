...
static int InputTextCalcTextLenAndLineCount(
    const char* text_begin, const char** out_text_end);
    ...
    if(g.DragDropActive && (flags & ImGuiButtonFlags_PressedOnDragDropHold) &&
        ...
        if(IsItemHovered(ImGuiHoveredFlags_AllowWhenBlockedByActiveItem))
        {
            ...
            if(CalcTypematicRepeatAmount(
                   ...
                   g.HoveredIdTimer + 0.0001f, DRAG_DROP_HOLD_TIMER, 0.00f))
            {
                pressed = true;
...
bool ImGui::ButtonEx(
    const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
...
{
    return window->GetIDNoKeepAlive(
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis,
    float* p_scroll_v, float size_avail_v, float size_contents_v,
    ImDrawCornerFlags rounding_corners)
{
    ...
    if(held && allow_interaction && grab_h_norm < 1.0f)
    {
        ...
        const float scroll_v_norm =
            ImSaturate((clicked_v_norm - g.ScrollbarClickDeltaToGrabCenter -
                       ...
                       (1.0f - grab_h_norm));
        *p_scroll_v = IM_ROUND(
            scroll_v_norm * scroll_max); //(win_size_contents_v - win_size_v));
    ...
    if(axis == ImGuiAxis_X)
        ...
        grab_rect = ImRect(bb.Min.x, ImLerp(bb.Min.y, bb.Max.y, grab_v_norm),
            bb.Max.x, ImLerp(bb.Min.y, bb.Max.y, grab_v_norm) + grab_h_pixels);
    ...
    const ImVec2& padding, const ImVec4& bg_col, const ImVec4& tint_col)
{
    ...
    const ImRect bb(
        window->DC.CursorPos, window->DC.CursorPos + size + padding * 2);
    ItemSize(bb);
    if(!ItemAdd(bb, id)) return false;
    ...
    RenderFrame(bb.Min, bb.Max, col, true,
        ImClamp(
            (float)ImMin(padding.x, padding.y), 0.0f, g.Style.FrameRounding));
    if(bg_col.w > 0.0f)
        window->DrawList->AddRectFilled(
    ...
    const ImVec4& bg_col, const ImVec4& tint_col)
{
    ...
    return ImageButtonEx(
        id, user_texture_id, size, uv0, uv1, padding, bg_col, tint_col);
}
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const ImRect total_bb(pos,
        pos +
            ImVec2(square_sz + (label_size.x > 0.0f
    ...
    float fraction, const ImVec2& size_arg, const char* overlay)
{
    ...
    ImVec2 size = CalcItemSize(
        size_arg, CalcItemWidth(), g.FontSize + style.FramePadding.y * 2.0f);
...
void ImGui::SeparatorEx(ImGuiSeparatorFlags flags)
{
    ...
    {
        // Vertical separator, for menu bars (use current line height). Not
        // exposed because it is misleading and it doesn't have an effect on
        // regular layout.
        ...
        window->DrawList->AddLine(ImVec2(bb.Min.x, bb.Min.y),
            ImVec2(bb.Min.x, bb.Max.y), GetColorU32(ImGuiCol_Separator));
...
void ImGui::ShrinkWidths(
    ImGuiShrinkWidthItem* items, int count, float width_excess)
{
    if(count == 1)
    {
    ...
    while(width_excess > 0.0f && count_same_width < count)
    {
        ...
        float max_width_to_remove_per_item =
            (count_same_width < count)
                ? (items[0].Width - items[count_same_width].Width)
                : (items[0].Width - 1.0f);
        float width_to_remove_per_item = ImMin(
    ...
    const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    if(!(flags & ImGuiComboFlags_NoPreview))
        window->DrawList->AddRectFilled(frame_bb.Min,
            ImVec2(value_x2, frame_bb.Max.y), frame_col, style.FrameRounding,
    ...
    if(!(flags & ImGuiComboFlags_NoArrowButton))
    {
        ...
        window->DrawList->AddRectFilled(ImVec2(value_x2, frame_bb.Min.y),
            frame_bb.Max, bg_col, style.FrameRounding,
    ...
    if(preview_value != NULL && !(flags & ImGuiComboFlags_NoPreview))
        RenderTextClipped(frame_bb.Min + style.FramePadding,
            ImVec2(value_x2, frame_bb.Max.y), preview_value, NULL, NULL,
            ImVec2(0.0f, 0.0f));
    if(label_size.x > 0)
    ...
    if(ImGuiWindow* popup_window = FindWindowByName(name))
        if(popup_window->WasActive)
        {
            ...
            ImVec2 pos = FindBestWindowPosForPopupEx(frame_bb.GetBL(),
                size_expected, &popup_window->AutoPosLastDirection, r_outer,
...
int ImGui::DataTypeFormatString(char* buf, int buf_size,
    ImGuiDataType data_type, const void* p_data, const char* format)
{
...
bool ImGui::DataTypeApplyOpFromText(const char* buf,
    const char* initial_value_buf, ImGuiDataType data_type, void* p_data,
    const char* format)
{
...
bool ImGui::DataTypeClamp(
    ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max)
{
    switch(data_type)
    {
        case ImGuiDataType_S8:
            return ClampBehaviorT<ImS8>(
                (ImS8*)p_data, *(const ImS8*)p_min, *(const ImS8*)p_max);
        case ImGuiDataType_U8:
            return ClampBehaviorT<ImU8>(
                (ImU8*)p_data, *(const ImU8*)p_min, *(const ImU8*)p_max);
        case ImGuiDataType_S16:
            return ClampBehaviorT<ImS16>(
                (ImS16*)p_data, *(const ImS16*)p_min, *(const ImS16*)p_max);
        case ImGuiDataType_U16:
            return ClampBehaviorT<ImU16>(
                (ImU16*)p_data, *(const ImU16*)p_min, *(const ImU16*)p_max);
        case ImGuiDataType_S32:
            return ClampBehaviorT<ImS32>(
                (ImS32*)p_data, *(const ImS32*)p_min, *(const ImS32*)p_max);
        case ImGuiDataType_U32:
            return ClampBehaviorT<ImU32>(
                (ImU32*)p_data, *(const ImU32*)p_min, *(const ImU32*)p_max);
        case ImGuiDataType_S64:
            return ClampBehaviorT<ImS64>(
                (ImS64*)p_data, *(const ImS64*)p_min, *(const ImS64*)p_max);
        case ImGuiDataType_U64:
            return ClampBehaviorT<ImU64>(
                (ImU64*)p_data, *(const ImU64*)p_min, *(const ImU64*)p_max);
        case ImGuiDataType_Float:
            return ClampBehaviorT<float>(
                (float*)p_data, *(const float*)p_min, *(const float*)p_max);
        case ImGuiDataType_Double:
            return ClampBehaviorT<double>(
...
bool ImGui::DragBehaviorT(ImGuiDataType data_type, TYPE* v, float v_speed,
    const TYPE v_min, const TYPE v_max, const char* format, float power,
    ImGuiDragFlags flags)
{
    ...
    const bool is_power = (power != 1.0f && is_decimal && is_clamped &&
                           (v_max - v_min < FLT_MAX));
    ...
    if(is_power)
    {
        ...
        v_cur = v_min +
                (SIGNEDTYPE)ImPow(ImSaturate((float)v_new_norm_curved), power) *
                    (v_max - v_min);
    ...
    float power, ImGuiDragFlags flags)
{
    ...
    switch(data_type)
    {
        case ImGuiDataType_S8:
        {
            ...
            bool r = DragBehaviorT<ImS32, ImS32, float>(ImGuiDataType_S32, &v32,
                v_speed, p_min ? *(const ImS8*)p_min : IM_S8_MIN,
        ...
        case ImGuiDataType_U8:
        {
            ...
            bool r = DragBehaviorT<ImU32, ImS32, float>(ImGuiDataType_U32, &v32,
                v_speed, p_min ? *(const ImU8*)p_min : IM_U8_MIN,
        ...
        case ImGuiDataType_S16:
        {
            ...
            bool r = DragBehaviorT<ImS32, ImS32, float>(ImGuiDataType_S32, &v32,
                v_speed, p_min ? *(const ImS16*)p_min : IM_S16_MIN,
                p_max ? *(const ImS16*)p_max : IM_S16_MAX, format, power,
                flags);
        ...
        case ImGuiDataType_U16:
        {
            ...
            bool r = DragBehaviorT<ImU32, ImS32, float>(ImGuiDataType_U32, &v32,
                v_speed, p_min ? *(const ImU16*)p_min : IM_U16_MIN,
                p_max ? *(const ImU16*)p_max : IM_U16_MAX, format, power,
                flags);
            if(r) *(ImU16*)p_v = (ImU16)v32;
            return r;
        }
        case ImGuiDataType_S32:
            return DragBehaviorT<ImS32, ImS32, float>(data_type, (ImS32*)p_v,
                v_speed, p_min ? *(const ImS32*)p_min : IM_S32_MIN,
                p_max ? *(const ImS32*)p_max : IM_S32_MAX, format, power,
                flags);
        case ImGuiDataType_U32:
            return DragBehaviorT<ImU32, ImS32, float>(data_type, (ImU32*)p_v,
                v_speed, p_min ? *(const ImU32*)p_min : IM_U32_MIN,
                p_max ? *(const ImU32*)p_max : IM_U32_MAX, format, power,
                flags);
        case ImGuiDataType_S64:
            return DragBehaviorT<ImS64, ImS64, double>(data_type, (ImS64*)p_v,
                v_speed, p_min ? *(const ImS64*)p_min : IM_S64_MIN,
                p_max ? *(const ImS64*)p_max : IM_S64_MAX, format, power,
                flags);
        case ImGuiDataType_U64:
            return DragBehaviorT<ImU64, ImS64, double>(data_type, (ImU64*)p_v,
                v_speed, p_min ? *(const ImU64*)p_min : IM_U64_MIN,
                p_max ? *(const ImU64*)p_max : IM_U64_MAX, format, power,
                flags);
        case ImGuiDataType_Float:
            return DragBehaviorT<float, float, float>(data_type, (float*)p_v,
                v_speed, p_min ? *(const float*)p_min : -FLT_MAX,
                p_max ? *(const float*)p_max : FLT_MAX, format, power, flags);
        case ImGuiDataType_Double:
            return DragBehaviorT<double, double, double>(data_type,
                (double*)p_v, v_speed, p_min ? *(const double*)p_min : -DBL_MAX,
    ...
    float power)
{
    ...
    if(power != 1.0f)
        IM_ASSERT(
            p_min != NULL && p_max != NULL); // When using a power curve the
                                             // drag needs to have known bounds
    ...
    if(temp_input_is_active || temp_input_start)
        return TempInputScalar(
            frame_bb, id, label, data_type, p_data, format); // , p_min, p_max);
    ...
    const bool value_changed = DragBehavior(id, data_type, p_data, v_speed,
        p_min, p_max, format, power, ImGuiDragFlags_None);
    ...
    const void* p_max, const char* format, float power)
{
    ...
    for(int i = 0; i < components; i++)
    {
        ...
        value_changed |= DragScalar(
            "", data_type, p_data, v_speed, p_min, p_max, format, power);
        PopID();
        PopItemWidth();
...
bool ImGui::DragFloat2(const char* label, float v[2], float v_speed,
    float v_min, float v_max, const char* format, float power)
{
...
bool ImGui::DragFloatRange2(const char* label, float* v_current_min,
    ...
    const char* format, const char* format_max, float power)
{
    ImGuiWindow* window = GetCurrentWindow();
    if(window->SkipItems) return false;
    ...
    bool value_changed = DragFloat("##min", v_current_min, v_speed,
    ...
    const char* format_max)
{
    ...
    bool value_changed = DragInt("##min", v_current_min, v_speed,
...
float ImGui::SliderCalcRatioFromValueT(ImGuiDataType data_type, TYPE v,
    TYPE v_min, TYPE v_max, float power, float linear_zero_pos)
{
...
bool ImGui::SliderBehaviorT(const ImRect& bb, ImGuiID id,
    ImGuiDataType data_type, TYPE* v, const TYPE v_min, const TYPE v_max,
    const char* format, float power, ImGuiSliderFlags flags,
    ImRect* out_grab_bb)
{
    ImGuiContext& g = *GImGui;
    const ImGuiStyle& style = g.Style;
    ...
    if(!is_decimal &&
        ...
        grab_sz = ImMax((float)(slider_sz / (v_range + 1)),
            style.GrabMinSize); // For integer sliders: if possible have the
                                // grab size represent 1 unit
...
bool ImGui::SliderBehavior(const ImRect& bb, ImGuiID id,
    ImGuiDataType data_type, void* p_v, const void* p_min, const void* p_max,
    const char* format, float power, ImGuiSliderFlags flags,
    ImRect* out_grab_bb)
{
    switch(data_type)
    {
        ...
        {
            ImS32 v32 = (ImS32) * (ImS8*)p_v;
            bool r = SliderBehaviorT<ImS32, ImS32, float>(bb, id,
                ImGuiDataType_S32, &v32, *(const ImS8*)p_min,
        ...
        {
            ImU32 v32 = (ImU32) * (ImU8*)p_v;
            bool r = SliderBehaviorT<ImU32, ImS32, float>(bb, id,
                ImGuiDataType_U32, &v32, *(const ImU8*)p_min,
        ...
        case ImGuiDataType_S16:
        {
            ImS32 v32 = (ImS32) * (ImS16*)p_v;
            bool r = SliderBehaviorT<ImS32, ImS32, float>(bb, id,
                ImGuiDataType_S32, &v32, *(const ImS16*)p_min,
        ...
        case ImGuiDataType_U16:
        {
            ImU32 v32 = (ImU32) * (ImU16*)p_v;
            bool r = SliderBehaviorT<ImU32, ImS32, float>(bb, id,
                ImGuiDataType_U32, &v32, *(const ImU16*)p_min,
        ...
        case ImGuiDataType_S32:
            ...
            return SliderBehaviorT<ImS32, ImS32, float>(bb, id, data_type,
                (ImS32*)p_v, *(const ImS32*)p_min, *(const ImS32*)p_max, format,
                power, flags, out_grab_bb);
        case ImGuiDataType_U32:
            IM_ASSERT(*(const ImU32*)p_max <= IM_U32_MAX / 2);
            return SliderBehaviorT<ImU32, ImS32, float>(bb, id, data_type,
                (ImU32*)p_v, *(const ImU32*)p_min, *(const ImU32*)p_max, format,
                power, flags, out_grab_bb);
        case ImGuiDataType_S64:
            IM_ASSERT(*(const ImS64*)p_min >= IM_S64_MIN / 2 &&
                      *(const ImS64*)p_max <= IM_S64_MAX / 2);
            return SliderBehaviorT<ImS64, ImS64, double>(bb, id, data_type,
                (ImS64*)p_v, *(const ImS64*)p_min, *(const ImS64*)p_max, format,
                power, flags, out_grab_bb);
        case ImGuiDataType_U64:
            IM_ASSERT(*(const ImU64*)p_max <= IM_U64_MAX / 2);
            return SliderBehaviorT<ImU64, ImS64, double>(bb, id, data_type,
                (ImU64*)p_v, *(const ImU64*)p_min, *(const ImU64*)p_max, format,
                power, flags, out_grab_bb);
        case ImGuiDataType_Float:
            IM_ASSERT(*(const float*)p_min >= -FLT_MAX / 2.0f &&
                      *(const float*)p_max <= FLT_MAX / 2.0f);
            return SliderBehaviorT<float, float, float>(bb, id, data_type,
                (float*)p_v, *(const float*)p_min, *(const float*)p_max, format,
                power, flags, out_grab_bb);
        case ImGuiDataType_Double:
            IM_ASSERT(*(const double*)p_min >= -DBL_MAX / 2.0f &&
                      *(const double*)p_max <= DBL_MAX / 2.0f);
            return SliderBehaviorT<double, double, double>(bb, id, data_type,
                (double*)p_v, *(const double*)p_min, *(const double*)p_max,
    ...
    float power)
{
    ...
    if(!temp_input_is_active)
    {
            ...
            g.NavInputId == id)
        {
            ...
            g.ActiveIdUsingNavDirMask |=
                (1 << ImGuiDir_Left) | (1 << ImGuiDir_Right);
            if(focus_requested || (clicked && g.IO.KeyCtrl) ||
                g.NavInputId == id)
            {
                temp_input_start = true;
                FocusableItemUnregister(window);
    ...
    // Slider behavior
    ImRect grab_bb;
    const bool value_changed = SliderBehavior(frame_bb, id, data_type, p_data,
        p_min, p_max, format, power, ImGuiSliderFlags_None, &grab_bb);
    if(value_changed) MarkItemEdited(id);
...
bool ImGui::SliderScalarN(const char* label, ImGuiDataType data_type, void* v,
    int components, const void* v_min, const void* v_max, const char* format,
    float power)
{
    ImGuiWindow* window = GetCurrentWindow();
    if(window->SkipItems) return false;
    ...
    const char* format, float power)
{
    ...
    ImRect grab_bb;
    const bool value_changed = SliderBehavior(frame_bb, id, data_type, p_data,
        p_min, p_max, format, power, ImGuiSliderFlags_Vertical, &grab_bb);
    if(value_changed) MarkItemEdited(id);
...
const char* ImParseFormatTrimDecorations(
    ...
    const void* p_clamp_min, const void* p_clamp_max)
{
    ...
    DataTypeFormatString(
        data_buf, IM_ARRAYSIZE(data_buf), data_type, p_data, format);
    ...
    if(p_step != NULL)
    {
        ...
        if(InputText("", buf, IM_ARRAYSIZE(buf),
            ...
            value_changed = DataTypeApplyOpFromText(buf,
                g.InputTextState.InitialTextA.Data, data_type, p_data, format);
...
static ImVec2 InputTextCalcTextSizeW(const ImWchar* text_begin,
    const ImWchar* text_end, const ImWchar** remaining, ImVec2* out_offset,
...
static float STB_TEXTEDIT_GETWIDTH(
    STB_TEXTEDIT_STRING* obj, int line_start_idx, int char_idx)
{
...
static void STB_TEXTEDIT_LAYOUTROW(
    StbTexteditRow* r, STB_TEXTEDIT_STRING* obj, int line_start_idx)
{
...
static void stb_textedit_replace(STB_TEXTEDIT_STRING* str,
    STB_TexteditState* state, const STB_TEXTEDIT_CHARTYPE* text, int text_len)
{
...
void ImGuiInputTextCallbackData::InsertChars(
    int pos, const char* new_text, const char* new_text_end)
{
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf,
    int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags,
    ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    const ImRect frame_bb(
        window->DC.CursorPos, window->DC.CursorPos + frame_size);
    ...
    if(g.ActiveId != id && init_make_active)
    {
        ...
        if(is_multiline)
            g.ActiveIdUsingKeyInputMask |=
                ...
                ((ImU64)1
                    << ImGuiKey_PageDown); // FIXME-NAV: Page up/down actually
                                           // not supported yet by widget, but
                                           // claim them ahead.
        if(flags &
            (ImGuiInputTextFlags_CallbackCompletion |
                ImGuiInputTextFlags_AllowTabInput)) // Disable keyboard tabbing
                                                    // out as we will use the \t
                                                    // character.
    ...
    if(g.ActiveId == id)
    {
        ...
        // It is ill-defined whether the back-end needs to send a \t character
        // when pressing the TAB keys. Win32 and GLFW naturally do it but not
        // SDL.
    ...
    if(g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id)
    {
        ...
        IM_ASSERT(io.KeyMods == GetMergedKeyModFlags() &&
                  ...
                  "io.KeyMods"); // We rarely do this check, but if anything
                                 // let's do it here.
        ...
        const bool is_shortcut_key =
            g.IO.ConfigMacOSXBehaviors ? (io.KeyMods == ImGuiKeyModFlags_Super)
        ...
        else if(IsKeyPressedMap(ImGuiKey_UpArrow) && is_multiline)
        {
            if(io.KeyCtrl)
                ...
                state->OnKeyPressed(
                    (is_startend_key_down ? STB_TEXTEDIT_K_TEXTSTART
                                          : STB_TEXTEDIT_K_UP) |
                    k_mask);
        }
        else if(IsKeyPressedMap(ImGuiKey_DownArrow) && is_multiline)
        {
            if(io.KeyCtrl)
                ...
                state->OnKeyPressed(
                    (is_startend_key_down ? STB_TEXTEDIT_K_TEXTEND
                                          : STB_TEXTEDIT_K_DOWN) |
                    k_mask);
        }
        ...
        {
            state->OnKeyPressed(
                is_undo ? STB_TEXTEDIT_K_UNDO : STB_TEXTEDIT_K_REDO);
                ...
                if(event_flag)
                {
                    ...
                    if(callback_data.BufDirty)
                    {
                        ...
                        state->CurLenW = ImTextStrFromUtf8(state->TextW.Data,
                            state->TextW.Size, callback_data.Buf, NULL);
                        state->CurLenA =
                            callback_data
    ...
    if(render_cursor || render_selection)
    {
        ...
        if(render_cursor && state->CursorFollow)
        {
            ...
            if(!(flags & ImGuiInputTextFlags_NoHorizontalScroll))
            {
                ...
                if(cursor_offset.x < state->ScrollX)
                    ...
                    state->ScrollX = IM_FLOOR(
                        cursor_offset.x - inner_size.x + scroll_increment_x);
            }
            else
            {
                state->ScrollX = 0.0f;
        ...
        if(render_selection)
        {
            ...
            for(const ImWchar* p = text_selected_begin; p < text_selected_end;)
            {
                ...
                if(rect_pos.y < clip_rect.y)
                {
                    ...
                    // wchar_t are 16-bit p = p ? p + 1 : text_selected_end;
            ...
            (buf_display_end - buf_display) < buf_display_max_length)
        {
            ImU32 col = GetColorU32(
                is_displaying_hint ? ImGuiCol_TextDisabled : ImGuiCol_Text);
            draw_window->DrawList->AddText(g.Font, g.FontSize,
                draw_pos - draw_scroll, col, buf_display, buf_display_end, 0.0f,
        ...
        if(render_cursor)
        {
            ...
            if(cursor_is_visible && cursor_screen_rect.Overlaps(clip_rect))
                draw_window->DrawList->AddLine(cursor_screen_rect.Min,
                    cursor_screen_rect.GetBL(), GetColorU32(ImGuiCol_Text));
    ...
    if(is_multiline)
    {
        Dummy(text_size +
              ImVec2(
                  0.0f, g.FontSize)); // Always add room to scroll an extra line
        EndChild();
        EndGroup();
    }
    ...
    const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    {
        // Hue is lost when converting from greyscale rgb (saturation=0).
        // Restore it.
        ...
        (flags & ImGuiColorEditFlags_NoInputs) == 0)
    {
        ...
        const bool hide_prefix =
            (w_item_one <=
                CalcTextSize(
                    (flags & ImGuiColorEditFlags_Float) ? "M:0.000" : "M:000")
                    .x);
    ...
    ImDrawList* draw_list, ImVec2 pos, ImVec2 half_sz, float bar_w, float alpha)
{
    ...
    ImGui::RenderArrowPointingAt(draw_list, ImVec2(pos.x + half_sz.x, pos.y),
        half_sz, ImGuiDir_Right, IM_COL32(255, 255, 255, alpha8));
    ImGui::RenderArrowPointingAt(draw_list,
        ImVec2(pos.x + bar_w - half_sz.x - 1, pos.y),
        ImVec2(half_sz.x + 2, half_sz.y + 1), ImGuiDir_Left,
        IM_COL32(0, 0, 0, alpha8));
    ImGui::RenderArrowPointingAt(draw_list,
        ImVec2(pos.x + bar_w - half_sz.x, pos.y), half_sz, ImGuiDir_Left,
    ...
    ImGuiColorEditFlags flags, const float* ref_col)
{
                   ...
                   ImRotate(initial_off, cos_hue_angle, sin_hue_angle)))
            {
                ...
                if(!ImTriangleContainsPoint(triangle_pa, triangle_pb,
                    ...
                    current_off_unrotated = ImTriangleClosestPoint(triangle_pa,
                        triangle_pb, triangle_pc, current_off_unrotated);
                float uu, vv, ww;
                ImTriangleBarycentricCoords(triangle_pa, triangle_pb,
                    triangle_pc, current_off_unrotated, uu, vv, ww);
    ...
    else if(flags & ImGuiColorEditFlags_PickerHueBar)
    {
        ...
        {
            S = ImSaturate(
                (io.MousePos.x - picker_pos.x) / (sv_picker_size - 1));
        ...
        {
            H = ImSaturate(
                (io.MousePos.y - picker_pos.y) / (sv_picker_size - 1));
            value_changed = value_changed_h = true;
        }
    }
    ...
    if(value_changed_h || value_changed_sv)
    {
        if(flags & ImGuiColorEditFlags_InputRGB)
        {
            ColorConvertHSVtoRGB(H >= 1.0f ? H - 10 * 1e-6f : H,
                S > 0.0f ? S : 10 * 1e-6f, V > 0.0f ? V : 1e-6f, col[0], col[1],
                col[2]);
    ...
    const ImU32 col_hues[6 + 1] = {IM_COL32(255, 0, 0, style_alpha8),
        IM_COL32(255, 255, 0, style_alpha8), IM_COL32(0, 255, 0, style_alpha8),
        IM_COL32(0, 255, 255, style_alpha8), IM_COL32(0, 0, 255, style_alpha8),
    ...
    if(flags & ImGuiColorEditFlags_PickerHueWheel)
    {
        ...
        for(int n = 0; n < 6; n++)
        {
            ...
            ShadeVertsLinearColorGradientKeepAlpha(draw_list, vert_start_idx,
                vert_end_idx, gradient_p0, gradient_p1, col_hues[n],
                col_hues[n + 1]);
        }
        ...
        draw_list->AddCircle(hue_cursor_pos, hue_cursor_rad + 1, col_midgrey,
            hue_cursor_segments);
        draw_list->AddCircle(
            hue_cursor_pos, hue_cursor_rad, col_white, hue_cursor_segments);
    ...
    else if(flags & ImGuiColorEditFlags_PickerHueBar)
    {
        ...
        draw_list->AddRectFilledMultiColor(picker_pos,
            picker_pos + ImVec2(sv_picker_size, sv_picker_size), col_white,
    ...
    draw_list->AddCircleFilled(
        sv_cursor_pos, sv_cursor_rad, user_col32_striped_of_alpha, 12);
    ...
    if(alpha_bar)
    {
        ...
        RenderColorRectWithAlphaCheckerboard(draw_list, bar1_bb.Min,
            bar1_bb.Max, 0, bar1_bb.GetWidth() / 2.0f, ImVec2(0.0f, 0.0f));
    ...
    ImGuiColorEditFlags flags, ImVec2 size)
{
    ...
    if(flags & ImGuiColorEditFlags_InputHSV)
        ColorConvertHSVtoRGB(
            col_rgb.x, col_rgb.y, col_rgb.z, col_rgb.x, col_rgb.y, col_rgb.z);
    ...
    if((flags & ImGuiColorEditFlags_AlphaPreviewHalf) && col_rgb.w < 1.0f)
    {
        ...
        window->DrawList->AddRectFilled(bb_inner.Min,
            ImVec2(mid_x, bb_inner.Max.y), GetColorU32(col_rgb_without_alpha),
    ...
    else
    {
        ...
        if(col_source.w < 1.0f)
            RenderColorRectWithAlphaCheckerboard(window->DrawList, bb_inner.Min,
                bb_inner.Max, GetColorU32(col_source), grid_step,
                ImVec2(off, off), rounding);
        else
            window->DrawList->AddRectFilled(bb_inner.Min, bb_inner.Max,
                GetColorU32(col_source), rounding, ImDrawCornerFlags_All);
    }
    RenderNavHighlight(bb, id);
    if((flags & ImGuiColorEditFlags_NoBorder) == 0)
    {
...
void ImGui::ColorTooltip(
    const char* text, const float* col, ImGuiColorEditFlags flags)
{
    ImGuiContext& g = *GImGui;
...
void ImGui::ColorEditOptionsPopup(const float* col, ImGuiColorEditFlags flags)
{
    ...
    if(BeginPopup("Copy"))
    {
        ...
        {
            ImFormatString(
                buf, IM_ARRAYSIZE(buf), "#%02X%02X%02X%02X", cr, cg, cb, ca);
            if(Selectable(buf)) SetClipboardText(buf);
        }
        EndPopup();
    }
...
void ImGui::ColorPickerOptionsPopup(
    ...
    const char* label, const char* label_end)
{
    ...
    const ImVec2 padding =
        (display_frame || (flags & ImGuiTreeNodeFlags_FramePadding))
            ? style.FramePadding
            : ImVec2(
                  style.FramePadding.x, ImMin(window->DC.CurrLineTextBaseOffset,
                                            style.FramePadding.y));
    ...
    if(display_frame)
    {
        // Framed header expand a little outside the default padding, to the
        // edge of InnerClipRect (FIXME: May remove this at some point and make
        // InnerClipRect align with WindowPadding.x instead of
    ...
    // subtle effect on _OpenOnDoubleClick: arrow click reacts on Down rather
    // than Up. It is rather standard that arrow click react on Down rather than
    // Up and we'd be tempted to make it the default (by removing the
    // _OpenOnArrow test below), however this would have a perhaps surprising
    // effect on CollapsingHeader()? So right now we are making this optional.
    // May evolve later. We set ImGuiButtonFlags_PressedOnClickRelease on
    // OpenOnDoubleClick because we want the item to be active on the initial
    ...
    // In this branch, TreeNodeBehavior() cannot toggle the selection so this
    // will never trigger.
        ...
        const ImU32 bg_col =
            GetColorU32((held && hovered) ? ImGuiCol_HeaderActive
                                          : hovered ? ImGuiCol_HeaderHovered
                                                    : ImGuiCol_Header);
        RenderFrame(
            frame_bb.Min, frame_bb.Max, bg_col, true, style.FrameRounding);
    ...
    const char* label, bool* p_open, ImGuiTreeNodeFlags flags)
{
    ...
    if(p_open != NULL)
    {
        ...
        float button_x = ImMax(window->DC.LastItemRect.Min.x,
            window->DC.LastItemRect.Max.x - g.Style.FramePadding.x * 2.0f -
                button_size);
    ...
    ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    bool hovered, held;
    bool pressed =
        ButtonBehavior(bb_enlarged, id, &hovered, &held, button_flags);
    ...
    if(hovered || selected)
    {
        ...
        RenderNavHighlight(bb_enlarged, id,
            ImGuiNavHighlightFlags_TypeThin |
...
bool ImGui::ListBoxHeader(
    const char* label, int items_count, int height_in_items)
{
...
bool ImGui::ListBox(const char* label, int* current_item,
    const char* const items[], int items_count, int height_items)
{
    ...
    float scale_max, ImVec2 frame_size)
{
    ...
    const ImRect inner_bb(
        frame_bb.Min + style.FramePadding, frame_bb.Max - style.FramePadding);
    const ImRect total_bb(frame_bb.Min,
        frame_bb.Max + ImVec2(label_size.x > 0.0f
                                  ? style.ItemInnerSpacing.x + label_size.x
                                  : 0.0f,
...
void ImGui::EndMenuBar()
{
        ...
        (g.NavWindow->Flags & ImGuiWindowFlags_ChildMenu))
    {
            ...
            g.NavMoveRequestForward == ImGuiNavForward_None)
        {
            ...
            FocusWindow(window);
            SetNavIDWithRectRel(
                window->NavLastIds[layer], layer, 0, window->NavRectRel[layer]);
            g.NavLayer = layer;
...
bool ImGui::BeginMainMenuBar()
{
    ...
    SetNextWindowSize(ImVec2(
        g.IO.DisplaySize.x, g.NextWindowData.MenuBarOffsetMinVal.y +
    ...
    const char* label, const char* shortcut, bool selected, bool enabled)
{
    ...
    else
    {
        ...
        float min_w =
            window->DC.MenuColumns.DeclColumns(label_size.x, shortcut_w,
                IM_FLOOR(g.FontSize * 1.20f)); // Feedback for next frame
        float extra_w = ImMax(0.0f, GetContentRegionAvail().x - min_w);
        pressed = Selectable(label, false,
            flags | ImGuiSelectableFlags_SpanAvailWidth, ImVec2(min_w, 0.0f));
        if(shortcut_w > 0.0f)
        {
            PushStyleColor(
                ImGuiCol_Text, g.Style.Colors[ImGuiCol_TextDisabled]);
            RenderText(
                pos + ImVec2(window->DC.MenuColumns.Pos[1] + extra_w, 0.0f),
                shortcut, NULL, false);
            PopStyleColor();
        }
        if(selected)
...
static int IMGUI_CDECL TabItemComparerByVisibleOffset(
...
bool ImGui::BeginTabBarEx(
    ImGuiTabBar* tab_bar, const ImRect& tab_bar_bb, ImGuiTabBarFlags flags)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
    if(window->SkipItems) return false;
...
void ImGui::EndTabBar()
{
    ...
    if(tab_bar == NULL)
    {
        IM_ASSERT_USER_ERROR(
            tab_bar != NULL, "Mismatched BeginTabBar()/EndTabBar()!");
        return;
    }
...
static void ImGui::TabBarLayout(ImGuiTabBar* tab_bar)
{
        ...
        (tab_bar->Flags & ImGuiTabBarFlags_FittingPolicyResizeDown))
    {
        ...
        ShrinkWidths(
            g.ShrinkWidthBuffer.Data, g.ShrinkWidthBuffer.Size, width_excess);
        for(int tab_n = 0; tab_n < tab_bar->Tabs.Size; tab_n++)
    ...
    tab_bar->OffsetNextTab =
        offset_x; // This is used by non-reorderable tab bar where the
...
static void ImGui::TabBarScrollToTab(ImGuiTabBar* tab_bar, ImGuiTabItem* tab)
{
    ...
    else if(tab_bar->ScrollingTarget < tab_x2 - tab_bar->BarRect.GetWidth())
    {
        tab_bar->ScrollingTargetDistToVisibility = ImMax(
            (tab_x1 - tab_bar->BarRect.GetWidth()) - tab_bar->ScrollingAnim,
            0.0f);
        tab_bar->ScrollingTarget = tab_x2 - tab_bar->BarRect.GetWidth();
    }
}

void ImGui::TabBarQueueChangeTabOrder(
    ImGuiTabBar* tab_bar, const ImGuiTabItem* tab, int dir)
{
...
static ImGuiTabItem* ImGui::TabBarScrollingButtons(ImGuiTabBar* tab_bar)
{
    ...
    bool want_clip_rect = !avail_bar_rect.Contains(ImRect(window->DC.CursorPos,
    ...
    const char* label, bool* p_open, ImGuiTabItemFlags flags)
{
    ...
    if(tab_bar == NULL)
    {
        IM_ASSERT_USER_ERROR(tab_bar,
            "BeginTabItem() Needs to be called between BeginTabBar() and "
            "EndTabBar()!");
        return false;
    }
    ...
    ImGuiTabItemFlags flags)
{
    ...
    {
        PushItemFlag(
            ImGuiItemFlags_NoNav | ImGuiItemFlags_NoNavDefaultFocus, true);
        ItemAdd(ImRect(), id);
        PopItemFlag();
        return false;
    }
...
void ImGui::TabItemBackground(
    ImDrawList* draw_list, const ImRect& bb, ImGuiTabItemFlags flags, ImU32 col)
{
    ...
    ImGuiID tab_id, ImGuiID close_button_id, bool is_contents_visible)
{
    ...
    if(flags & ImGuiTabItemFlags_UnsavedDocument)
    {
        ...
        RenderTextClippedEx(draw_list, unsaved_marker_pos,
            bb.Max - frame_padding, TAB_UNSAVED_MARKER, NULL, NULL);
    }
    ...
    RenderTextEllipsis(draw_list, text_ellipsis_clip_bb.Min,
        text_ellipsis_clip_bb.Max, text_pixel_clip_bb.Max.x, ellipsis_max_x,
...
void ImGui::SetWindowClipRectBeforeSetChannel(
    ImGuiWindow* window, const ImRect& clip_rect)
{
...
float ImGui::GetColumnOffsetFromNorm(
    const ImGuiColumns* columns, float offset_norm)
{
...
static float GetColumnWidthEx(
    ImGuiColumns* columns, int column_index, bool before_resize = false)
{
    if(column_index < 0) column_index = columns->Current;
...
void ImGui::SetColumnOffset(int column_index, float offset)
{
    ...
    if(preserve_width)
        SetColumnOffset(
            column_index + 1, offset + ImMax(g.Style.ColumnsMinSpacing, width));
}
...
void ImGui::BeginColumns(
    const char* str_id, int columns_count, ImGuiColumnsFlags flags)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = GetCurrentWindow();
    ...
    if(!(flags & ImGuiColumnsFlags_NoBorder) && !window->SkipItems)
    {
        ...
        for(int n = 1; n < columns->Count; n++)
        {
            ...
            const ImRect column_hit_rect(
                ImVec2(x - column_hit_hw, y1), ImVec2(x + column_hit_hw, y2));
            ...
            window->DrawList->AddLine(
                ImVec2(xi, y1 + 1.0f), ImVec2(xi, y2), col);
        }
